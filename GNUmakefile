...
#### Changed OSDETECT to OSTARGET ####
ifdef OSDETECT_DONTRUNTEST
	OSTARGET_DONTRUNTEST=$(OSDETECT_DONTRUNTEST)
endif
ifdef OSDETECT_WINDOWS
	OSTARGET_WINDOWS=$(OSDETECT_WINDOWS)
endif
ifdef OSDETECT_LINUX
	OSTARGET_LINUX=$(OSDETECT_LINUX)
endif
ifdef OSDETECT_FREEBSD
	OSTARGET_FREEBSD=$(OSDETECT_FREEBSD)
endif
ifdef OSDETECT_MSDOS
	OSTARGET_MSDOS=$(OSDETECT_MSDOS)
endif
ifdef OSDETECT_MACOS
	OSTARGET_MACOS=$(OSDETECT_MACOS)
endif
######################################

# We must detect what OS we are running on
ifndef OSTARGET_DONTRUNTEST
...
ifdef SUBSYSTEM_SDL				### SDL
	ifdef OSTARGET_WINDOWS		# Windows has problems, no sdl-config
		ifdef SDL_INCLUDE
		...
		ifdef SDL_LIB
			CC_SUBSYSTEMLFLAGS = -L$(SDL_LIB) -lSDL.dll
		else
			CC_SUBSYSTEMLFLAGS = -L. -LSDL -lSDL.dll
		endif
	else
		ifdef DONTUSESDLCONFIG
			ifdef SDL_INCLUDE
				CC_SUBSYSTEMCFLAGS = -I$(SDL_INCLUDE)
			else
				CC_SUBSYSTEMCFLAGS = -I. -ISDL
			...
			CC_SUBSYSTEMCFLAGS = $(shell sdl-config --cflags) -DDIRECTFULLSCREEN -DSDL -DVID_X11
			CC_SUBSYSTEMLFLAGS = $(shell sdl-config --libs) -DDIRECTFULLSCREEN -DSDL -DVID_X11
		endif
	endif
...
else ifdef SUBSYSTEM_SERVER
	CC_SUBSYSTEMCFLAGS = 
	CC_SUBSYSTEMLFLAGS = -lm
	
	CC_SUBSYSTEMDIRECTORY = server
endif
...
ifdef OSTARGET_WINDOWS
	CC_SYSTEMCFLAGS = -DWIN32 -D_WIN32 -DENABLEMULTITHREADING
	CC_SYSTEMLFLAGS = -mwindows -ldxguid -ldinput -ldsound -lddraw -lwinmm -lwsock32 -lkernel32 -luser32 -lgdi32 -lwinspool -lcomdlg32 -ladvapi32 -lshell32 -lole32 -loleaut32 -luuid -lodbc32 -lodbccp32
	CC_OBJBASE      = w
else ifdef OSTARGET_MSDOS
	CC_SYSTEMCFLAGS = -D__MSDOS__
	CC_SYSTEMLFLAGS = 
	CC_OBJBASE      = d
else ifdef OSTARGET_LINUX
	CC_SYSTEMCFLAGS = -DLINUX -DENABLEMULTITHREADING
	CC_SYSTEMLFLAGS = -pthread
	CC_OBJBASE      = l
else ifdef OSTARGET_FREEBSD
	CC_SYSTEMCFLAGS = -DFREEBSD  -DENABLEMULTITHREADING
	CC_SYSTEMLFLAGS = -pthread
	CC_OBJBASE      = f
else ifdef OSTARGET_MACOS
	CC_SYSTEMCFLAGS = 
	CC_SYSTEMLFLAGS = 
	CC_OBJBASE      = m
endif
...
# Server?
ifdef SERVER
	CC_SERVERCFLAGS = -DGAMESERVER -UGAMECLIENT
	RC_SERVERFLAGS = -DGAMESERVER -UGAMECLIENT
else
	CC_SERVERCFLAGS = -DGAMECLIENT -UGAMESERVER
	RC_SERVERFLAGS = -DGAMECLIENT -UGAMESERVER
endif

ifdef CC_MANUALFLAGS
	CC_CFLAGS = $(CC_EXTRACFLAGS)
	CC_LFLAGS = $(CC_EXTRALFLAGS)
else
	CC_BASEBASECFLAGS = $(CC_EXTRACFLAGS) $(CC_SERVERCFLAGS) $(CC_SYSTEMCFLAGS) $(CC_MODECFLAGS) -I$(INPUT_SRC) -I$(INPUT_ISRC)
	CC_BASEBASELFLAGS = $(CC_EXTRALFLAGS) $(CC_SYSTEMLFLAGS) $(CC_MODELFLAGS)
	
	ifdef SERVER
		CC_CFLAGS = $(CC_BASEBASECFLAGS) -I$(INPUT_SVSRC)
		CC_LFLAGS = $(CC_BASEBASELFLAGS)
	else
		CC_CFLAGS = $(CC_BASEBASECFLAGS) $(CC_SUBSYSTEMCFLAGS) -I$(INPUT_CLSRC)
		CC_LFLAGS = $(CC_BASEBASELFLAGS) $(CC_SUBSYSTEMLFLAGS)
	endif
endif
...
ifdef SERVER
	CC_EXEOUTPUT3      = $(CC_EXEOUTPUT2)-server
else
	CC_EXEOUTPUT3      = $(CC_EXEOUTPUT2)
endif

ifdef DEBUG
	CC_EXEOUTPUT1      = $(CC_EXEOUTPUT3)-dbg
	CC_LAUNCHEROUTPUT1 = $(CC_LAUNCHEROUTPUT2)-dbg
else
	CC_EXEOUTPUT1      = $(CC_EXEOUTPUT3)
...
ifdef DEBUG
	ifdef SERVER
		CC_OBJPREFIX = $(CC_OBJPREFIX2)sd
	else
		CC_OBJPREFIX = $(CC_OBJPREFIX2)d
	endif
else
	ifdef SERVER
		CC_OBJPREFIX = $(CC_OBJPREFIX2)sr
	else
...
CC_CLIENTOBJS = $(CC_OBJPREFIX)am_map.o \
			...
			$(CC_OBJPREFIX)consolex.o 

CC_SERVEROBJS = $(CC_OBJPREFIX)consolex.o

CC_COMMONOBJS = $(SYSTEMOBJECTS) \
...
ifdef SERVER
	CC_OBJS = $(CC_SERVEROBJS) $(CC_COMMONOBJS)
else
	CC_OBJS = $(CC_CLIENTOBJS) $(CC_COMMONOBJS)
endif
...
$(CC_OBJPREFIX)launcherrc.o: util/launcher/w32_lnch.rc
		$(WINDRES) -DMINGWMAKED --input-format=rc -o $(CC_OBJPREFIX)launcherrc.o util/launcher/w32_lnch.rc -O coff $(RC_FLAGS) $(RC_SERVERFLAGS)

endif

ifneq (,$(or $(OSHOST_LINUX), $(OSTARGET_LINUX)))
...
ifndef DEBIANMAINTAINER
	DEBIANMAINTAINER = "Unknown"
endif
...
debian:		wad release
			...
			echo "Maintainer: $(DEBIANMAINTAINER) ($(DEBIANEMAIL))" >> debian/DEBIAN/control
			...
			echo " stability and innovations. It is currently available for" >> debian/DEBIAN/control
			echo " Windows 98/98SE/ME/NT/2000/XP/2003/Vista/2008/7, ReactOS 0.3+" >> debian/DEBIAN/control
			echo " and Linux. It also has the ability to work on both 32-bit and" >> debian/DEBIAN/control
			echo " 64-bit platforms." >> debian/DEBIAN/control
...
$(CC_OBJPREFIX)%.o:	$(INPUT_SRC)/%.c
					$(CC) -c $< -o $@ $(CC_CFLAGS)
ifdef SERVER
$(CC_OBJPREFIX)%.o:	$(INPUT_SVSRC)/%.c
					$(CC) -c $< -o $@ $(CC_CFLAGS)
else
$(CC_OBJPREFIX)%.o:	$(INPUT_CLSRC)/%.c
					$(CC) -c $< -o $@ $(CC_CFLAGS)
endif
