...
static bool DS_RMODReadToken(D_RMODTokenInfo_t* const a_Info)
{
	uint16_t wc, wcLast;
	int Action;
	bool Flipped;
...
static bool DS_RMODPDC(const struct WL_WADFile_s* const a_WAD, const uint32_t a_Key, void** const a_DataPtr, size_t* const a_SizePtr, WL_RemoveFunc_t* const a_RemoveFuncPtr)
{
#define BUFSIZE 384
	...
	static bool CheckRecursive = false;
	...
	*a_DataPtr = Stuff = (D_RMODWADStuff_t*)Z_Malloc(*a_SizePtr, PU_WLDKRMOD, NULL);
			...
			for (i = 0; i < NUMDRMODPRIVATES; i++)
				if (strcasecmp(TokVals[1], c_RMODHandlers[i].TableType) == 0)
				{
					...
					if (c_RMODHandlers[i].HandleFunc)
						if (!c_RMODHandlers[i].HandleFunc(CurrentTable, a_WAD, (D_RMODPrivates_t)i, &Stuff->Private[i]))
...
static bool DS_RMODOCCB(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
	int i;
	...
	for (i = 0; i < NUMDRMODPRIVATES; i++)
		if (c_RMODHandlers[i].OrderFunc)
			if (!c_RMODHandlers[i].OrderFunc(a_Pushed, a_WAD, (D_RMODPrivates_t)i))
				if (devparm)
...
D_RMODPrivate_t* D_GetRMODPrivate(const WL_WADFile_t* const a_WAD, const D_RMODPrivates_t a_ID)
{
	...
	Stuff = (D_RMODWADStuff_t*)WL_GetPrivateData(a_WAD, WLDK_RMOD, NULL);
...
char* D_RMODGetValueString(Z_Table_t* const a_Table, const char* const a_Value, char* const a_MissingVal)
{
	...
	/* Check */
	if (!a_Table || !a_Value)
		return Z_StrDup(a_MissingVal, PU_WLDKRMOD, NULL);
