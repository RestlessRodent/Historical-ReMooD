...
struct I_NetSocket_s
{
	...
	bool BadOnlySix;							// Failed to make it IPv6 Only
	bool TCPConnected;						// Connected with TCP
	bool DidAcceptSix;						// Accepted IPv6 connection
...
static bool l_IPv6 = false;					// using IPv6 Mode
...
I_NetSocket_t* I_NetOpenSocket(const bool a_Server, const I_HostAddress_t* const a_Host, const uint16_t a_Port)
{
	...
	NewSock = (I_NetSocket_t*)Z_Malloc(sizeof(*NewSock), PU_STATIC, NULL);
...
static size_t IS_NetRecvWrap(I_NetSocket_t* const a_Socket, I_HostAddress_t* const a_Host, void* const a_OutData, const size_t a_Len, const bool a_Peek, const bool a_CheckConn)
{
#if defined(__MSDOS__)
	return 0;
#else
...
size_t I_NetSend(I_NetSocket_t* const a_Socket, const I_HostAddress_t* const a_Host, const void* const a_InData, const size_t a_Len)
{
	...
	// Convert address to host
	if (a_Host)
		IS_NetAddrWrapToNative(a_Host, (sockaddr_in*)(!i ? &Addr : NULL), (sockaddr_in6*)(i ? &Addr : NULL));
