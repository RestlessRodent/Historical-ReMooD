...
bool PTR_FindCameraPoint(intercept_t* in, void* a_Data)
{
...
// added : now angle turn is a absolute value not relative
	...
	else
	{
		...
		{
			// Change direction
			player->mo->movedir = (dirtype_t)((((uint32_t)player->cmd.angleturn & 0xE000) >> 13U));
			
			// Odd?
			if ((player->cmd.angleturn >> 12) & 1)
				player->mo->movedir = (dirtype_t)((int)player->mo->movedir + 1);
			
			// Correct direction
			if (player->mo->movedir < 0)
				player->mo->movedir = (dirtype_t)((int)DI_NODIR - 1);
			else if (player->mo->movedir >= DI_NODIR)
				player->mo->movedir = (dirtype_t)0;
