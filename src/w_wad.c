	...
	if (M_CheckParm("-waddir"))
		while (M_IsNextParm())
		{
			...
			if (NumDirs < MAXSEARCHPATH)
			{
				...
				while (strlen(SearchDirs[NumDirs]) && (SearchDirs[NumDirs][strlen(SearchDirs[NumDirs]) - 1] == '/'
#if defined(_WIN32)
		...
		if (n)
		{
			...
			switch (n->Method)
			{
				case METHOD_WAD:	// WAD File
					...
					{
						if (devparm)
							CONS_Printf("W_LoadWadFile: Failed! NumLumps ?= %i (read %i bytes)\n", NumLumps, readcount);
						fclose(tFile);
						return -5;
					}
					...
					{
						if (devparm)
							CONS_Printf("W_LoadWadFile: Failed! IndexOffset ?= %i (read %i bytes)\n", IndexOffset, readcount);
						fclose(tFile);
						return -6;
					}
