			...
			if (!--door->topcountdown)
			{
				switch (door->type)
				{
					...
					case genBlazeRaise:	//SoM: 3/6/2000
						...
						S_StartSound((S_NoiseThinker_t*)&door->sector->soundorg, sfx_bdcls);
						break;
						
					case normalDoor:
					case genRaise:	//SoM: 3/6/2000
						door->direction = -1;	// time to go back down
						S_StartSound((S_NoiseThinker_t*)&door->sector->soundorg, doorclosesound);
						break;
						
					case close30ThenOpen:
					case genCdO:	//SoM: 3/6/2000
						door->direction = 1;
						S_StartSound((S_NoiseThinker_t*)&door->sector->soundorg, sfx_doropn);
						break;
						
						//SoM: 3/6/2000
					case genBlazeCdO:
						door->direction = 1;	// time to go back up
						S_StartSound((S_NoiseThinker_t*)&door->sector->soundorg, sfx_bdopn);
						break;
...
void EV_OpenDoor(int sectag, int speed, int wait_time)
{
	...
	while ((secnum = P_FindSectorFromTag(sectag, secnum)) >= 0)
	{
		...
		if (door->topheight != sec->ceilingheight)
			S_StartSound((S_NoiseThinker_t*)&door->sector->soundorg, speed >= 4 ? sfx_bdopn : sfx_doropn);
	}
}
...
void EV_CloseDoor(int sectag, int speed)
{
	...
	while ((secnum = P_FindSectorFromTag(sectag, secnum)) >= 0)
	{
		...
		S_StartSound((S_NoiseThinker_t*)&door->sector->soundorg, speed >= 4 ? sfx_bdcls : sfx_dorcls);
	}
}
...
//    side = 0;   // only front sides can be used
	...
	switch (line->special)
	{
		...
		case 118:				// BLAZING DOOR OPEN
			S_StartSound((S_NoiseThinker_t*)&sec->soundorg, sfx_bdopn);
			break;
			
		case 1:				// NORMAL DOOR SOUND
		case 31:
			S_StartSound((S_NoiseThinker_t*)&sec->soundorg, sfx_doropn);
			break;
