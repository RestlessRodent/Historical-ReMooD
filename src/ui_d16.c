...
// ----------------------------------------------------------------------------
// DESCRIPTION: 16-bit (High Color) Drawers
...
void UI_d16_DrawImg(UI_BufferSpec_t* const a_Spec, UI_Img_t* const a_Img, const int32_t a_X, const int32_t a_Y)
{
	...
	/* Draw loop */
	Dest = ((uint16_t*)a_Spec->Data) + (uintptr_t)((y1 * a_Spec->p) + x1);
	Src = ((uint16_t*)a_Img->Data) + (uintptr_t)((oy * a_Img->p) + ox);
	for (y = y1; y < y2; y++)
	{
		memcpy(Dest, Src, (x2 - x1) * sizeof(uint16_t));
		
		Dest = (uint16_t*)((uintptr_t)Dest + a_Spec->pd);
		Src = (uint16_t*)((uintptr_t)Src + a_Img->pd);
	}
}
