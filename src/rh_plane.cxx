...
RH_VisPlane_t* RH_FindPlane(RH_RenderStat_t* const a_Stat, RH_VisPlane_t* const a_Input)
{
	...
	a_Stat->VisPlanes[Hash][a_Stat->NumVisPlanes[Hash]++] = New = (RH_VisPlane_t*)Z_Malloc(sizeof(*New), PU_STATIC, NULL);
	...
	/* Make buffers */
	// Top
	New->Top = (int16_t*)Z_Malloc(sizeof(*New->Top) * (vid.width + 2), PU_STATIC, NULL);
	New->Top = &New->Top[1];
	memset(New->Top, 0xff, sizeof(*New->Top) * (vid.width + 2));
	
	// Bottom
	New->Bottom = (int16_t*)Z_Malloc(sizeof(*New->Bottom) * (vid.width + 2), PU_STATIC, NULL);
