...
struct CONL_ConVariable_s
{
	...
	bool IsVirtual;							// A virtual variable (not regged)
	bool LoadedValue;							// Was set in the config file
	bool IsDeprecated;						// Is really a deprecated variable
...
static bool l_CONLVarLoaded = false;						// Loaded config
...
// A = const char*
// B = uintptr_t
static bool CONL_CommandHashCompare(void* const a_A, void* const a_B)
{
	/* Check */
	...
	/* Caseless compare */
	if (strcasecmp((const char*)a_A, l_CONLCommands[((uintptr_t)a_B) - 1].Name) == 0)
		return true;
...
// A = const char*
// B = uintptr_t
static bool CONL_VariableHashCompare(void* const a_A, void* const a_B)
{
	const char* A;
...
static CONL_ConVariable_t* CONLS_VarCoreLocate(const char* const a_Name)
{
	...
	/* Find in table */
	Found = (CONL_ConVariable_t*)Z_HashFindEntry(l_CONLVariableHashes, Hash, (void*)a_Name, false);
...
static CONL_ConVariable_t* CONLS_PushVar(const char* const a_Name, CONL_StaticVar_t* const a_StaticVar)
{
	...
	// Create
	NewVar = (CONL_ConVariable_t*)Z_Malloc(sizeof(*NewVar), PU_STATIC, NULL);
...
bool CONL_AddCommand(const char* const a_Name, CONL_ExitCode_t (*a_ComFunc)(const uint32_t, const char** const))
{
	...
	// Add to hash
	Z_HashAddEntry(l_CONLCommandHashes, l_CONLCommands[l_CONLNumCommands].Hash, (void*)((uintptr_t)(l_CONLNumCommands + 1)));
...
CONL_ExitCode_t CONL_Exec(const uint32_t a_ArgC, const char** const a_ArgV)
{
	...
	ComNum = (uintptr_t)Z_HashFindEntry(l_CONLCommandHashes, Hash, (void*)(a_ArgV[0]), false);
	ComNum -= 1;
...
#define BUFSIZE 256
	...
	else
	{
		...
		IntVal = strtol(a_NewVal, (char**)&StoppedAt, 10);
