...
void P_InitPicAnims(void)
{
	...
	anims = (anim_t*)Z_Malloc(sizeof(*anims) * (maxanims + 1), PU_STATIC, NULL);
...
void P_ActivateCrossedLine(line_t* line, int side, mobj_t* thing)
{
	int ok;
	bool UseAgain;
	bool forceuse;				//SoM: 4/26/2000: ALLTRIGGER should allow monsters to use generalized types too!
	...
	for (i = 0; i < MAXBUTTONS; i++)
		if (buttonlist[i].btimer)
		{
			...
			if (!buttonlist[i].btimer)
			{
				...
				S_StartSound((S_NoiseThinker_t*)&buttonlist[i].soundorg, sfx_swtchn);
...
static bool PS_RMODSpecialEffects(Z_Table_t* const a_Sub, void* const a_Data)
{
}
...
bool P_RMODH_Specials(Z_Table_t* const a_Table, const WL_WADFile_t* const a_WAD, const D_RMODPrivates_t a_ID, D_RMODPrivate_t* const a_Private)
{
	...
	/* Set data area */
	Specs = (P_RMODSpecials_t*)RealPrivate->Data;
...
bool P_RMODO_Specials(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD, const D_RMODPrivates_t a_ID)
{
	...
	for (RoveWAD = WL_IterateVWAD(NULL, true); RoveWAD; RoveWAD = WL_IterateVWAD(RoveWAD, true))
	{
		...
		// Get specials
		Specials = (P_RMODSpecials_t*)RMODPrivate->Data;
...
P_RMODTouchSpecial_t* P_RMODTouchSpecialForSprite(const uint32_t a_SprNum)
{
	...
	return (P_RMODTouchSpecial_t*)g_SprTouchSpecials[a_SprNum];
}
...
static bool PS_ExtraSpecialOCCB(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
#define BUFSIZE 512
	const WL_WADEntry_t* Entry;
