...
static void P_NewChaseDir(mobj_t* actor)
{
	...
	if (P_Random() & 1)
	{
		for (tdi = (int)DI_EAST; tdi <= (int)DI_SOUTHEAST; tdi++)
		{
			tdir = (dirtype_t)tdi;
	...
	{
		for (tdi = (int)DI_SOUTHEAST; tdi >= (int)DI_EAST; tdi--)
		{
			tdir = (dirtype_t)tdi;
...
bool PIT_VileCheck(mobj_t* thing, void* a_Arg)
{
	int maxdist;
	bool check;
	mobj_t* TheVile = (mobj_t*)a_Arg;
...
static state_t* P_FinalState(statenum_t state)
{
	...
	if (NUMSTATES != OldStateCount || !final_state)
	{
		if (final_state)
			Z_Free(final_state);
		final_state = (char*)Z_Malloc(sizeof(*final_state) * NUMSTATES, PU_STATIC, NULL);
