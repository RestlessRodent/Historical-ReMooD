...
typedef struct S_SoundChannel_s
{
	...
	bool Used;				// Channel is being used
...
bool S_VolumeVarsChanged(CONL_ConVariable_t* const a_Var, CONL_StaticVar_t* const a_StaticVar);
...
static bool l_SoundOK = false;	// Did the sound start OK?
static bool l_MusicOK = true;	// Same but for Music
...
static bool l_ThreadedSound = false;	// Threaded sound
...
static CONL_ExitCode_t SCLC_SoundMulti(const uint32_t a_ArgC, const char** const a_ArgV)
{
	...
	{
		S_ChangeMusicName((char*)a_ArgV[1], true);
	}
...
void S_Init(int sfxVolume, int musicVolume)
{
	...
	if (!M_CheckParm("-nosfx"))
	{
		...
		l_DoomChannels = (S_SoundChannel_t*)Z_Malloc(sizeof(*l_DoomChannels) * l_NumDoomChannels, PU_STATIC, NULL);
