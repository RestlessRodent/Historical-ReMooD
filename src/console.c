...
#define BUFSIZE 512
	...
	else if (a_Event->Type == IET_SYNTHOSK)
	{
		...
		Char = a_Event->Data.SynthOSK.Character;
	}
...
/* CONL_OSKDrawForPlayer() -- Draws OSK for Player */
void CONL_OSKDrawForPlayer(const int32_t a_PlayerNum)
{
	...
	/* Determine Dimensions and how to draw it */
	// Menu Input; For Player 1 in Single Screen (or no screen)
	if (Player == -1 || (g_SplitScreen <= 0 && Player == 0))
	{
		x = 80;
		y = 120;
		w = 320;
	...
	/* Draw */
	CONLS_DrawOSK(x, y, w, h, (Player < 0 ? 0 : Player));
}
...
bool_t CONL_OSKHandleEvent(const I_EventEx_t* const a_Event, const size_t a_PlayerNum)
{
	...
	// These are generated by the OSK themselves
	if (a_Event->Data.SynthOSK.KeyCode || a_Event->Data.SynthOSK.Character)
	{
	...
	if (a_Event->Data.SynthOSK.Direct & 0x800000)
	{
		...
		c = (a_Event->Data.SynthOSK.Direct & 0xFF0) >> 4;
