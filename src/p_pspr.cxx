...
{
	weaponinfo_t* Weapon = (weaponinfo_t*)a_Input;	
...
bool P_RMODH_WeaponsAmmo(Z_Table_t* const a_Table, const WL_WADFile_t* const a_WAD, const D_RMODPrivates_t a_ID, D_RMODPrivate_t* const a_Private)
{
	...
	// Get the local stuff
	LocalStuff = (P_LocalWeaponsAndAmmo_t*)RealPrivate->Data;
	...
	if (a_ID == DRMODP_ITEMAMMO)
	{
		...
		LocalStuff->Ammo[LocalStuff->NumAmmo] = (ammoinfo_t*)Z_Malloc(sizeof(*LocalStuff->Ammo[LocalStuff->NumAmmo]), PU_WLDKRMOD, NULL);
	...
	else if (a_ID == DRMODP_ITEMWEAPON)
	{
		...
		LocalStuff->Weapons[LocalStuff->NumWeapons] = (weaponinfo_t*)Z_Malloc(sizeof(*LocalStuff->Weapons[LocalStuff->NumWeapons]), PU_WLDKRMOD, NULL);
...
bool P_RMODO_WeaponsAmmo(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD, const D_RMODPrivates_t a_ID)
{
	...
	for (RoveWAD = WL_IterateVWAD(NULL, true); RoveWAD; RoveWAD = WL_IterateVWAD(RoveWAD, true))
	{
		...
		// Load menu stuff
		LocalStuff = (P_LocalWeaponsAndAmmo_t*)RMODPrivate->Data;
