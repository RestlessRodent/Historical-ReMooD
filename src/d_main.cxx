...
bool g_DedicatedServer = true;				// Dedicated Server
#else
bool g_DedicatedServer = false;				// Dedicated Server
#endif
...
bool newnet_use = false;
bool newnet_solo = false;
...
static bool l_FPSPanic = false;

/* D_SetFPSPanicMode() -- Try to get more FPS */
void D_SetFPSPanicMode(const bool a_Set)
{
	l_FPSPanic = a_Set;
}
...
typedef struct D_IWADInfoEx_s
{
	...
	bool CanDistrib;							// Distributable? (Not illegal to give away)
...
static bool DS_DetectReMooDWAD(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
	...
	g_RandomData = (uint8_t*)Z_Malloc(g_RandomDataSize, PU_STATIC, NULL);
...
static bool DS_DetectGameMode(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
	int32_t* Confidence;
	int32_t TotalScore;
	...
	Confidence = (int32_t*)Z_Malloc(sizeof(*Confidence) * NumConf, PU_STATIC, NULL);
...
void D_DoomMain(void)
{
	...
	/* Adapters */
	V_ImageFindA(NULL, VCP_NONE);		// Bump image loaders
	...
	if (p && p < myargc - 1)
	{
		gameskill = (skill_t)(myargv[p + 1][0] - '1');
		autostart = true;
	}
