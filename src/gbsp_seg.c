...
static glbsp_intersection_t *quick_alloc_cuts = NULL;

//
// NewIntersection
//
static glbsp_intersection_t *NewIntersection(void)
{
	glbsp_intersection_t *cut;
	...
	{
		cut = UtilCalloc(sizeof(glbsp_intersection_t));
	}
...
void FreeQuickAllocCuts(void)
{
	...
	{
		glbsp_intersection_t *cut = quick_alloc_cuts;
...
// AddIntersection
//
static void AddIntersection(glbsp_intersection_t ** cut_list, glbsp_vertex_t * vert, glbsp_seg_t * part, bool_t self_ref)
{
	glbsp_intersection_t *cut;
	glbsp_intersection_t *after;
...
//
static int EvalPartitionWorker(glbsp_superblock_t * seg_list, glbsp_seg_t * part, int best_cost, eval_info_t * info)
{
	glbsp_seg_t *check;
...
// skipped altogether.
//
static int EvalPartition(glbsp_superblock_t * seg_list, glbsp_seg_t * part, int best_cost)
{
	eval_info_t info;
	...
	if (part->pdx != 0 && part->pdy != 0)
		info.cost += 25;
			   ...
			   info.real_left, info.mini_left, info.real_right, info.mini_right, info.cost / 100, info.cost % 100);
#endif

	return info.cost;
}

static glbsp_seg_t *FindSegFromStaleNode(glbsp_superblock_t * part_list, glbsp_glbsp_node_t * stale_nd, int *stale_opposite)
{
	glbsp_seg_t *part;
	int num;
	...
	for (part = part_list->segs; part; part = part->next)
	{
		...
		fb = fabs(UtilPerpDist(part, stale_nd->x + stale_nd->dx, stale_nd->y + stale_nd->dy));

		if (fa < DIST_EPSILON && fb < DIST_EPSILON)
		{
			/* OK found it.  check if runs in same direction */

			(*stale_opposite) = (stale_nd->dx * part->pdx + stale_nd->dy * part->pdy < 0) ? 1 : 0;
	...
	for (num = 0; num < 2; num++)
	{
		...
		part = FindSegFromStaleNode(part_list->subs[num], stale_nd, stale_opposite);
...
static int PickNodeWorker(glbsp_superblock_t * part_list, glbsp_superblock_t * seg_list, glbsp_seg_t ** best, int *best_cost, int *progress, int prog_step)
{
	glbsp_seg_t *part;
...
#if DEBUG_PICKNODE
				   part->linedef ? "" : "MINI", part, part->sector ? part->sector->index : -1, part->start->x, part->start->y, part->end->x, part->end->y);
#endif
	...
	{
		if (part_list->subs[num])
			PickNodeWorker(part_list->subs[num], seg_list, best, best_cost, progress, prog_step);
	}
...
glbsp_seg_t *PickNode(glbsp_superblock_t * seg_list, int depth, glbsp_glbsp_node_t ** stale_nd, int *stale_opposite)
{
	...
	if (false == PickNodeWorker(seg_list, seg_list, &best, &best_cost, &progress, prog_step))
	{
...
// SeparateSegs
//
void SeparateSegs(glbsp_superblock_t * seg_list, glbsp_seg_t * part, glbsp_superblock_t * lefts, glbsp_superblock_t * rights, glbsp_intersection_t ** cut_list)
{
	int num;
	...
	for (num = 0; num < 2; num++)
	{
		glbsp_superblock_t *A = seg_list->subs[num];
...
static void FindLimitWorker(glbsp_superblock_t * block, glbsp_bbox_t * bbox)
{
	glbsp_seg_t *cur;
	int num;
...
{
	glbsp_intersection_t *cur, *next;
	glbsp_seg_t *seg, *buddy;
	...
	for (cur = cut_list; cur; cur = cur->next)
	{
				   ...
				   cur->along_dist, cur->before ? cur->before->index : -1, cur->after ? cur->after->index : -1, cur->self_ref ? "SELFREF" : "");
	}
#endif
	...
	while(cur && next)
	{
		...
		// merge the two intersections into one
				   ...
				   next->vertex, next->vertex->x, next->vertex->y, next->before ? next->before->index : -1, next->after ? next->after->index : -1);
#endif
...
#if DEBUG_CUTLIST
				   cur->vertex->x, cur->vertex->y, cur->before ? cur->before->index : -1, cur->after ? cur->after->index : -1, cur->self_ref ? "SELFREF" : "");
#endif
	...
	for (cur = cut_list; cur && cur->next; cur = cur->next)
	{
		...
		if (cur->after && !next->before)
		{
			if (!cur->self_ref && !cur->after->warned_unclosed)
			{
				PrintMiniWarn("Sector #%d is unclosed near (%1.1f,%1.1f)\n",
							  cur->after->index, (cur->vertex->x + next->vertex->x) / 2.0, (cur->vertex->y + next->vertex->y) / 2.0);
		...
		else if (!cur->after && next->before)
		{
			...
			{
				PrintMiniWarn("Sector #%d is unclosed near (%1.1f,%1.1f)\n",
							  next->before->index, (cur->vertex->x + next->vertex->x) / 2.0, (cur->vertex->y + next->vertex->y) / 2.0);
		...
		if (cur->after != next->before)
		{
			if (!cur->self_ref && !next->self_ref)
				PrintMiniWarn("Sector mismatch: #%d (%1.1f,%1.1f) != #%d (%1.1f,%1.1f)\n",
							  cur->after->index, cur->vertex->x, cur->vertex->y, next->before->index, next->vertex->x, next->vertex->y);
...
#if DEBUG_CUTLIST
				   seg, seg->sector ? seg->sector->index : -1, seg->start->x, seg->start->y, seg->end->x, seg->end->y);

				   buddy, buddy->sector ? buddy->sector->index : -1, buddy->start->x, buddy->start->y, buddy->end->x, buddy->end->y);
#endif
	}
