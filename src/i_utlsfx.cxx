...
typedef struct I_LocalMusic_s
{
	...
	bool Playing;				// Is the song playing?
} I_LocalMusic_t;
...
bool I_MUS2MID_MUSReadNextMessage(I_MUS2MIDData_t* const a_Local, uint32_t* const a_OutData, size_t* const a_OutSize, uint32_t* const a_Delta)
{
	void* p;
	uint8_t NoteBit[4];
	bool Last;
...
bool I_MUS2MID_Init(struct I_MusicDriver_s* const a_Driver)
{
	...
	a_Driver->Data = Local = (I_MUS2MIDData_t*)Z_Malloc(a_Driver->Size, PU_STATIC, NULL);
...
bool I_MUS2MID_Destroy(struct I_MusicDriver_s* const a_Driver)
{
	I_MUS2MIDData_t* Local;
	...
	/* Get local */
	Local = (I_MUS2MIDData_t*)a_Driver->Data;
...
int I_MUS2MID_Play(struct I_MusicDriver_s* const a_Driver, const void* const a_Data, const size_t a_Size, const bool Loop)
{
	I_MUS2MIDData_t* Local;
	size_t i;
	union
...
typedef struct I_OPLEmData_s
{
	...
	bool init_stage_reg_writes;				// Intialization stage?
...
static bool IS_OPLEm_Init(struct I_MusicDriver_s* const a_Driver)
{
	int r;
	I_OPLEmData_t* Local;
	...
	/* Create Data */
	a_Driver->Data = Local = (I_OPLEmData_t*)Z_Malloc(sizeof(*Local), PU_STATIC, NULL);
...
static void IS_OPLEm_RawMIDI(struct I_MusicDriver_s* const a_Driver, const uint32_t a_Msg, const uint32_t a_BitLength)
{
	...
	/* Get Data */
	Local = (I_OPLEmData_t*)a_Driver->Data;
...
bool I_AddMusicDriver(I_MusicDriver_t* const a_Driver)
{
	...
	{
		// Resize the list
		Z_ResizeArray((void**)&l_MusicDrivers, sizeof(*l_MusicDrivers), l_NumMusicDrivers, l_NumMusicDrivers + 1);
...
#define BUFSIZE 512
	...
	New.Data = (uint8_t*)WX_CacheEntry(Entry);
	...
	{
		// Resize the list
		Z_ResizeArray((void**)&l_LocalSongs, sizeof(*l_LocalSongs), l_NumLocalSongs, l_NumLocalSongs + 1);
...
void I_UnRegisterSong(int handle)
{
	...
	// remaining, null
	l_LocalSongs[i].Type = (I_MusicType_t)0;
...
bool I_AddSoundDriver(I_SoundDriver_t* const a_Driver)
{
	...
	{
		// Resize the list
		Z_ResizeArray((void**)&l_SoundDrivers, sizeof(*l_SoundDrivers), l_NumSoundDrivers, l_NumSoundDrivers + 1);
