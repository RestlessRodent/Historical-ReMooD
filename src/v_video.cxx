...
bool V_VIDGammaOnChange(CONL_ConVariable_t* const a_Var, CONL_StaticVar_t* const a_StaticVar);
...
void LoadPalette(char* lumpname)
{
	...
	PlayPal = (uint8_t*)Z_Malloc(256 * 3 * 14 * sizeof(uint8_t), PU_STATIC, NULL);
	...
	l_DoomPals = (RGBA_t**)Z_Malloc(sizeof(*l_DoomPals) * (NumBasePals * VPALSMOOTHCOUNT), PU_STATIC, NULL);
	...
	for (j = 0, i = 0; i < NumBasePals; i++)
	{
		...
		l_DoomPals[k] = (RGBA_t*)Z_Malloc(sizeof(RGBA_t) * 256, PU_STATIC, NULL);
	...
	for (i = 0; i < l_NumDoomPals; i++)
	{
		...
		else
		{
			...
			if (!l_DoomPals[i])
				l_DoomPals[i] = (RGBA_t*)Z_Malloc(sizeof(RGBA_t) * 256, PU_STATIC, NULL);
	...
	// Re-Allocate
	l_MappedDoomPals = (uint8_t**)Z_Malloc(sizeof(*l_MappedDoomPals) * l_NumDoomPals, PU_STATIC, NULL);
	...
	for (i = 0; i < l_NumDoomPals; i++)
	{
		...
		l_MappedDoomPals[i] = MappedData = (uint8_t*)Z_Malloc(256, PU_STATIC, NULL);
...
void V_SetPalette(int palettenum)
{
	...
	if (!l_DoomPals)
	{
		FakePal = (RGBA_t*)Z_Malloc(sizeof(*FakePal) * 256, PU_STATIC, NULL);
...
uint8_t* V_GetPalette(int palettenum)
{
	...
	if (palettenum < 0 || palettenum >= l_NumDoomPals || !l_DoomPals[palettenum])
		return (uint8_t*)l_DoomPals[0];
	else
		return (uint8_t*)l_DoomPals[palettenum];
}
...
{
	if (palettenum < 0 || palettenum >= l_NumDoomPals || !l_DoomPals[palettenum])
		return (uint8_t*)l_MappedDoomPals[0];
	else
		return (uint8_t*)l_MappedDoomPals[palettenum];
}
...
static bool VS_VideoWADOrderCB(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
	/* Load PLAYPAL */
...
void V_InitializeColormaps(void)
{
	...
	l_LifeBarColors = (uint8_t*)Z_Malloc(sizeof(*l_LifeBarColors) * 200, PU_STATIC, NULL);
	...
	for (i = 0; i < (NUMVEXCOLORS * 2); i++)
	{
		// Does not exist?
		if (!l_ColorMaps[i])
			l_ColorMaps[i] = (uint8_t*)Z_Malloc(sizeof(*l_ColorMaps[i]) * 256, PU_STATIC, NULL);
...
void V_DrawFadeConsBackEx(const uint32_t Flags, const int x1, const int y1, const int x2, const int y2)
{
	...
	{
		// Set buf
		buf = (uint8_t*)(screens[0] + (vid.width * y) + X1);
		...
		{
			buf2 = (uint32_t*)(screens[0] + (vid.width * (y + i)) + X1);
...
void V_DrawColorBoxEx(const uint32_t a_Flags, const uint8_t a_Color, const int32_t a_x1, const int32_t a_y1, const int32_t a_x2, const int32_t a_y2)
{
	...
	uint8_t* buf, *buf2;
	uint32_t c, Mask;
	const uint8_t* Map, *ExtraMap, *TransMap;
	...
	{
		// Top Line
		buf = (uint8_t*)(screens[0] + (vid.width * (Y1)) + X1);
		memset(buf, c, X2 - X1);
		
		// Bottom Line
		buf = (uint8_t*)(screens[0] + (vid.width * (Y2)) + X1);
		memset(buf, c, X2 - X1);
	...
	else
	{
		...
		// Solid Box
			...
			for (y = Y1; y < Y2; y += 8)
			{
				...
				for (i = 1; i < 8 && (y + i) < Y2; i++)
				{
					buf2 = (uint8_t*)(screens[0] + (vid.width * (y + i)) + X1);
					memcpy(buf2, buf, X2 - X1);
				}
			}
	}
...
void V_DrawPatchEx(const uint32_t Flags, const int x, const int y, const patch_t* const Patch, const uint8_t* const ExtraMap)
{
	...
	ColorMap = V_ReturnColormapPtr((VEX_ColorList_t)Color);
...
static bool VS_DetectByName(const char* const a_Name, uint16_t* const a_HexOut, VideoFont_t* const a_FontOut, V_FontNameRule_t* const a_RuleOut)
{
	/*** DEDICATED SERVER ***/
...
static void VS_FontPDCRemove(const struct WL_WADFile_s* a_WAD)
{
	...
	/* Get local stuff */
	LocalStuff = (V_LocalFontStuff_t*)WL_GetPrivateData(a_WAD, WLDK_VFONTS, NULL);
...
static V_UniChar_t* VS_AddCharacter(const bool a_Local, V_LocalFontStuff_t* const a_LocalStuff, const WL_WADEntry_t* const a_Entry, const uint16_t a_Hex, const VideoFont_t a_Font, const uint16_t a_GAlias, const uint16_t* const a_GBuilder, V_UniChar_t* const a_CharP)
{
	/*** DEDICATED SERVER ***/
#if defined(__REMOOD_DEDICATED)
	return NULL;
	...
	if (a_Local)
	{
		...
		if (!a_LocalStuff->CGroups[a_Font])
			a_LocalStuff->CGroups[a_Font] = (V_UniChar_t**)Z_Malloc(sizeof(*a_LocalStuff->CGroups[a_Font]) * 256, PU_STATIC, (void**)&a_LocalStuff->CGroups[a_Font]);
		...
		if (!a_LocalStuff->CGroups[a_Font][Master])
			a_LocalStuff->CGroups[a_Font][Master] = (V_UniChar_t*)Z_Malloc(sizeof(*a_LocalStuff->CGroups[a_Font][Master]) * 256, PU_STATIC, (void**)&a_LocalStuff->CGroups[a_Font][Master]);
	...
	else
	{
		...
		if (!l_CGroups[a_Font])
			l_CGroups[a_Font] = (V_UniChar_t***)Z_Malloc(sizeof(*l_CGroups[a_Font]) * 256, PU_STATIC, NULL);
		...
		if (!l_CGroups[a_Font][Master])
			l_CGroups[a_Font][Master] = (V_UniChar_t**)Z_Malloc(sizeof(*l_CGroups[a_Font][Master]) * 256, PU_STATIC, NULL);
...
static bool VS_FontPDCCreate(const struct WL_WADFile_s* const a_WAD, const uint32_t a_Key, void** const a_DataPtr, size_t* const a_SizePtr, WL_RemoveFunc_t* const a_RemoveFuncPtr)
{
	/*** DEDICATED SERVER ***/
#if defined(__REMOOD_DEDICATED)
	return false;
	...
	*a_SizePtr = sizeof(*FontStuff);
	FontStuff = (V_LocalFontStuff_t*)Z_Malloc(*a_SizePtr, PU_STATIC, NULL);
	*a_DataPtr = FontStuff;
...
// composited table.
static bool VS_FontOCCB(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
	/*** DEDICATED SERVER ***/
#if defined(__REMOOD_DEDICATED)
	return false;
...
#define MAXHEIGHTCHECKERS	16
	...
	while ((WADRover = WL_IterateVWAD(WADRover, true)))
	{
		...
		FontStuff = (V_LocalFontStuff_t*)WL_GetPrivateData(WADRover, WLDK_VFONTS, NULL);
		...
		for (f = 0; f < NUMVIDEOFONTS; f++)
		{
			...
			{
				// Add this character
				if (VS_AddCharacter(false, FontStuff, Rover->Entry, Rover->Char, (VideoFont_t)f, 0, 0, Rover))
					Count++;
	...
	{
		// Allocate data
		UTData = (uint8_t*)Z_Malloc(UTTT->Size + 5, PU_STATIC, NULL);
					...
					do
					{
						...
						for (f = 0; f < NUMVIDEOFONTS; f++)
							if (l_CGroups[f])
							{
								...
								if (VS_AddCharacter(false, NULL, NULL, a, (VideoFont_t)f, 0, 0, l_CGroups[f][MasterC][SlaveC]))
									Count++;
							}
					} while (*p != 0);
					break;
	...
	for (f = 0; f < NUMVIDEOFONTS; f++)
	{
		...
		for (i = 0; VSpaceChars[i].Start; i++)
			for (j = 0; j < VSpaceChars[i].Len; i++)
			{
				...
				if (VS_AddCharacter(false, NULL, NULL, v, (VideoFont_t)f, 0, 0, &l_VSpace[f]))
					Count++;
			}
	}
...
static bool l_VSImageBooted = false;
...
uint8_t* VS_GetPalMap(V_Image_t* const a_Image)
{
	...
	if (!l_VSPalMap[0])
	{
		l_VSPalMap[0] = (uint8_t*)Z_Malloc(sizeof(*l_VSPalMap[0]) * 256, PU_STATIC, NULL);
		for (i = 0; i < 256; i++)
			l_VSPalMap[0][i] = i;
	}
	...
	if (l_VSGamePal != g_CoreGame)
	{
		...
		// 1:1 Mapping
		l_VSPalMap[OneOne] = (uint8_t*)Z_Malloc(sizeof(*l_VSPalMap[OneOne]) * 256, PU_STATIC, NULL);
		for (i = 0; i < 256; i++)
		...
		if (!Entry)
		{
			l_VSPalMap[TransA] = (uint8_t*)Z_Malloc(sizeof(*l_VSPalMap[TransA]) * 256, PU_STATIC, NULL);
			for (i = 0; i < 256; i++)
...
// a_A = const char* char
// a_B = V_Image_t* const
bool VS_HashImageCompare(void* const a_A, void* const a_B)
{
	const char* A;
	V_Image_t* B;
...
static bool VS_WLImagePDC(const struct WL_WADFile_s* const a_WAD, const uint32_t a_Key, void** const a_DataPtr, size_t* const a_SizePtr, WL_RemoveFunc_t* const a_RemoveFuncPtr)
{
	V_WLImageHolder_t* HI;
	
	/* Allocate */
	*a_SizePtr = sizeof(V_WLImageHolder_t);
	HI = (V_WLImageHolder_t*)Z_Malloc(*a_SizePtr, PU_STATIC, NULL);
	*a_DataPtr = HI;
...
static void VS_WLImagePDCRemove(const struct WL_WADFile_s* a_WAD)
{
	...
	/* Obtain */
	HI = (V_WLImageHolder_t*)WL_GetPrivateData(a_WAD, WLDK_VIMAGES, NULL);
...
#define HEADERSIZE 12
	...
	if (Header[0] > 0 && Header[1] > 0)			// w/h > 0
	{
		...
		Offs = (uint32_t*)Z_Malloc(sizeof(*Offs) * Header[0], PU_STATIC, NULL);
	...
	// Create blank image
	New = (V_Image_t*)Z_Malloc(sizeof(*New), PU_STATIC, NULL);
	...
	HI = (V_WLImageHolder_t*)WL_GetPrivateData(a_Entry->Owner, WLDK_VIMAGES, NULL);
...
V_Image_t* V_ImageFindA(const char* const a_Name, const V_ColorPal_t a_Pal)
{
	...
	while ((Rover = WL_IterateVWAD(Rover, false)))
	{
		...
		WADImages = (V_WLImageHolder_t*)WL_GetPrivateData(Rover, WLDK_VIMAGES, NULL);
		...
		// Look in hashes
		FoundImage = (V_Image_t*)Z_HashFindEntry(WADImages->ImageHashes, Hash, (void*)a_Name, false);
...
void V_ImageDestroy(V_Image_t* const a_Image)
{
	...
	/* Remove from WAD */
	HI = (V_WLImageHolder_t*)WL_GetPrivateData(a_Image->wData->Owner, WLDK_VIMAGES, NULL);
...
const struct patch_s* V_ImageGetPatch(V_Image_t* const a_Image, size_t* const a_ByteSize)
{
	...
	if (a_Image->NativeType == VIT_PATCH)
	{
		...
		a_Image->dPatch = (patch_s*)Z_Malloc(TotalSize, PU_STATIC, (void**)&a_Image->dPatch);
...
{	/*** DEDICATED SERVER ***/
	...
	else
	{
		...
		a_Image->dPic = (pic_s*)Z_Malloc(TotalSize, PU_STATIC, (void**)&a_Image->dPic);
...
uint8_t* V_ImageGetRaw(V_Image_t* const a_Image, size_t* const a_ByteSize)
{
	...
	if (a_Image->NativeType == VIT_RAW)
	{
		...
		a_Image->dRaw = (uint8_t*)Z_Malloc(TotalSize, PU_STATIC, (void**)&a_Image->dRaw);
	...
	else
	{
		...
		else if (a_Image->NativeType == VIT_PATCH)
		{
			...
			for (x = 0; x < a_Image->Width; x++)
			{
				...
				p = (uint8_t*)(((uintptr_t)Patch) + Patch->columnofs[x]);
...
#define __REMOOD_ONESHORT
	...
	const uint8_t* ColorMap;
	const uint8_t* ColorMapE;
	uint8_t* TransMap;
	uint8_t Pixel;
	
	const patch_t* RawPatch;
	...
	// Get it
	ColorMap = V_ReturnColormapPtr((VEX_ColorList_t)x);
	...
	else if (a_Flags & VEX_COLORSET)
	{
		...
		ColorMapE = V_ReturnColormapPtr((VEX_ColorList_t)(16 + Mask));
	}
	else
