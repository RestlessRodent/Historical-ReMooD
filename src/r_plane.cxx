...
visplane_t* new_visplane(unsigned hash)
{
	...
	if (!check)
	{
		check = (visplane_t*)Z_Malloc(sizeof(visplane_t), PU_STATIC, 0);
		check->top = (uint16_t*)Z_Malloc(sizeof(*check->top) * (vid.width + 2), PU_STATIC, NULL);
		check->top = &check->top[1];
		check->bottom = (uint16_t*)Z_Malloc(sizeof(*check->top) * (vid.width + 2), PU_STATIC, NULL);
