	...
	for (frame = 0; frame < maxframe; frame++)
	{
		switch ((int)sprtemp[frame].rotate)
		{
			case -1:
				...
				CONS_Printf("R_InitSprites: No patches found for %s frame %c",
							sprname, frame + 'A');
				return false;
				...
				for (rotation = 0; rotation < 8; rotation++)
					...
					if (sprtemp[frame].lumppat[rotation] == -1)
						I_Error("R_InitSprites: Sprite %s frame %c "
								"is missing rotations", sprname, frame + 'A');
				break;
		}
	}
...
static void R_SplitSprite(vissprite_t * sprite, mobj_t * thing)
{
	...
	{
		if (sector->lightlist[i].height >= sprite->gzt ||
			!(sector->lightlist[i].caster->flags & FF_CUTSPRITES))
			continue;
		...
		cutfrac =
			(centeryfrac -
			 FixedMul(sector->lightlist[i].height - viewz, sprite->scale)) >> FRACBITS;
		if (cutfrac < 0)
			continue;
...
static void R_ProjectSprite(mobj_t * thing)
{
	...
	if (thing->subsector->sector->numlights)
	{
		...
		if (thing->subsector->sector->lightlist[light].caster &&
			thing->subsector->sector->lightlist[light].caster->flags & FF_FOG)
			lightnum = (*thing->subsector->sector->lightlist[light].lightlevel >> LIGHTSEGSHIFT);
		else
			lightnum =
				(*thing->subsector->sector->lightlist[light].
	...
	if (heightsec != -1)		// only clip things which are in special sectors
	{
		...
		if (phs != -1 && viewz < sectors[phs].floorheight ?
			thing->z >= sectors[heightsec].floorheight : gzt < sectors[heightsec].floorheight)
			return;
	...
	else
	{
		...
		if (fixedcolormap)
		{
			...
			vis->colormap = fixedcolormap;
				  ...
				  (thing->flags & MF_SHADOW)) && (!vis->extra_colormap ||
												  !vis->extra_colormap->fog))
		{
			// full bright : goggles
...
void R_DrawPSprite(pspdef_t * psp)
{
	...
	if (viewplayer->mo->subsector->sector->numlights)
	{
		...
		lightnum =
			(*viewplayer->mo->subsector->sector->lightlist[light].
			 lightlevel >> LIGHTSEGSHIFT) + extralight;
...
void R_DrawPlayerSprites(void)
{
	...
	{
		light =
			R_GetPlaneLight(viewplayer->mo->subsector->sector,
							viewplayer->mo->z + viewplayer->mo->info->height, false);
		lightnum =
			(*viewplayer->mo->subsector->sector->lightlist[i].
			 lightlevel >> LIGHTSEGSHIFT) + extralight;
	}
	else
		...
		if (ds->numffloorplanes)
		{
			for (i = 0; i < ds->numffloorplanes; i++)
			{
				...
				for (p = 0; p < ds->numffloorplanes; p++)
				{
					...
					R_PlaneBounds(plane);
					if (plane->low < con_clipviewtop || plane->high > vid.height
	...
	for (rover = vsprsortedhead.prev; rover != &vsprsortedhead; rover = rover->prev)
	{
		...
		for (r2 = nodehead.next; r2 != &nodehead; r2 = r2->next)
		{
			...
			else if (r2->thickseg)
			{
				...
				scale =
					r2->thickseg->scale1 >
					r2->thickseg->scale2 ? r2->thickseg->scale1 : r2->thickseg->scale2;
				if (scale <= rover->scale)
					continue;
				scale =
					r2->thickseg->scale1 +
					(r2->thickseg->scalestep * (sintersect - r2->thickseg->x1));
				if (scale <= rover->scale)
					continue;
...
WadIndex_t W_CheckForSkinMarkerInPwad(int wadid, int startlump)
{
	...
	if (startlump < wad->NumLumps)
	{
		...
		{
			if (*((int *)wad->Index[i].Name) == v1 &&
				wad->Index[i].Name[4] == 'I' && wad->Index[i].Name[5] == 'N')
			{
