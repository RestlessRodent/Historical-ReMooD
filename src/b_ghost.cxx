...
// SubSector Mesh
static bool l_SSMCreated = false;				// Mesh created?
...
static bool BS_GHOSTCheckNodeTrav(intercept_t* in, void* const a_Data)
{
	line_t* li;
	mobj_t* mo;
...
B_GhostNode_t* B_GHOST_NodeNearPos(const fixed_t a_X, const fixed_t a_Y, const fixed_t a_Z, const bool a_Any)
{
...
B_GhostNode_t* B_GHOST_CreateNodeAtPos(const fixed_t a_X, const fixed_t a_Y)
{
	...
	/* Create new node */
	New = (B_GhostNode_t*)Z_Malloc(sizeof(*New), PU_BOTS, NULL);
...
bool B_GHOST_RecursiveSplitMap(const node_t* const a_Node)
{
	int8_t Side, i, j;
	fixed_t x1, y1, x2, y2;
...
static bool BS_GhostNTNPFirst(intercept_t* in, void* const a_Data)
{
	line_t* li;
...
static bool BS_CheckNodeToNode(B_GhostBot_t* const a_Bot, B_GhostNode_t* const a_Start, B_GhostNode_t* const a_End, const bool a_FirstTime)
{
	/* Check */
...
void B_GHOST_InitLevel(void)
{
	...
	l_BAdj = (sector_t* (*)[32])Z_Malloc(sizeof(*l_BAdj) * l_BNumSecs, PU_BOTS, NULL);
	l_BNumAdj = (size_t*)Z_Malloc(sizeof(*l_BNumAdj) * l_BNumSecs, PU_BOTS, NULL);
	...
	// Allocate
	l_UMGrid = (B_Unimatrix_t*)Z_Malloc(sizeof(*l_UMGrid) * (l_UMSize[0] * l_UMSize[1]), PU_BOTS, NULL);
...
static bool BS_GHOST_JOB_RandomNav(struct B_GhostBot_s* a_GhostBot, const size_t a_JobID)
{
...
static bool BS_GHOST_JOB_ShootStuff(struct B_GhostBot_s* a_GhostBot, const size_t a_JobID)
{
#define CLOSEMOS 8
	int32_t s, i, m;
	sector_t* CurSec;
...
static bool BS_GHOST_JOB_GunControl(struct B_GhostBot_s* a_GhostBot, const size_t a_JobID)
{
...
void B_GHOST_Think(B_GhostBot_t* const a_GhostBot, ticcmd_t* const a_TicCmd)
{
	...
	if (!a_GhostBot->Initted)
	{
		...
		// Randomize Posture
		a_GhostBot->AISpec.Posture = (B_GhostAtkPosture_t)(BS_Random(a_GhostBot) % NUMBGHOSTATKPOSTURE);
