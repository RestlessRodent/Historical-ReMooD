				...
				case -1:
					...
					if (P_EXGSGetValue(PEXGSBID_COBOOMSUPPORT) && sector->heightsec != -1 && sector->altheightsec == 1)
					{
						if ((sector->floorheight - speed) <
							...
							S_StartSound((S_NoiseThinker_t*)&sector->soundorg, sfx_gloop);
					}
					// Moving a floor down
...
void T_MoveFloor(floormove_t* floor)
{
	...
	if (!(leveltime % (8)))
		S_StartSound((S_NoiseThinker_t*)&floor->sector->soundorg, ceilmovesound);
	...
	if (res == pastdest)
	{
		...
		S_StartSound((S_NoiseThinker_t*)&floor->sector->soundorg, sfx_pstop);
	}
...
void T_MoveElevator(elevator_t* elevator)
{
	...
	if (elevator->type == EVGHEELEVT_STOPPERP)
	{
		...
		if (!elevator->Silent)
			S_StartSound((S_NoiseThinker_t*)&elevator->sector->soundorg, sfx_pstop);
	...
	else if (elevator->type == EVGHEELEVT_PERPUP || elevator->type == EVGHEELEVT_PERPDOWN)
	{
		...
		if (elevator->direction == 0)
		{
			...
			if (StartMove)
			{
				...
				if (!elevator->Silent)
					if (!elevator->Dinged)
					{
						S_StartSound((S_NoiseThinker_t*)&elevator->sector->soundorg, sfx_elvcal);
		...
		else
		{
			...
			if (!elevator->Silent)
				if (!(leveltime % (8)))
					S_StartSound((S_NoiseThinker_t*)&elevator->sector->soundorg, sfx_stnmov);
