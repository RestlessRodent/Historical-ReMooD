...
#define SCRFREE(x) if (x) { Z_Free(x); x = NULL; }
	...
	/* (Re)allocate */
	// Width
	xtoviewangle = (angle_t*)Z_Malloc(sizeof(angle_t) * (vid.width + 1), PU_STATIC, NULL);
	columnofs = (int*)Z_Malloc(sizeof(*columnofs) * vid.width, PU_STATIC, NULL);
	floorclip = (short int*)Z_Malloc(sizeof(*floorclip) * (vid.width + 1), PU_STATIC, NULL);
	ceilingclip = (short int*)Z_Malloc(sizeof(*ceilingclip) * (vid.width + 1), PU_STATIC, NULL);
	frontscale = (fixed_t*)Z_Malloc(sizeof(*frontscale) * vid.width, PU_STATIC, NULL);
	distscale = (fixed_t*)Z_Malloc(sizeof(*distscale) * vid.width, PU_STATIC, NULL);
#ifdef BORIS_FIX
	last_ceilingclip = (short int*)Z_Malloc(sizeof(*last_ceilingclip) * vid.width, PU_STATIC, NULL);
	last_floorclip = (short int*)Z_Malloc(sizeof(*last_floorclip) * vid.width, PU_STATIC, NULL);
#endif
	negonearray = (short int*)Z_Malloc(sizeof(*negonearray) * vid.width, PU_STATIC, NULL);
	screenheightarray = (short int*)Z_Malloc(sizeof(*screenheightarray) * vid.width, PU_STATIC, NULL);
	solidsegs = (cliprange_t*)Z_Malloc(sizeof(*solidsegs) * MAXSEGS, PU_STATIC, NULL);
	
	// Height
	ylookup = (uint8_t**)Z_Malloc(sizeof(uint8_t*) * vid.height, PU_STATIC, NULL);
	ylookup1 = (uint8_t**)Z_Malloc(sizeof(uint8_t*) * vid.height, PU_STATIC, NULL);
	ylookup2 = (uint8_t**)Z_Malloc(sizeof(uint8_t*) * vid.height, PU_STATIC, NULL);
	for (i = 0; i < MAXSPLITSCREENPLAYERS; i++)
		ylookup4[i] = (uint8_t**)Z_Malloc(sizeof(uint8_t*) * vid.height, PU_STATIC, NULL);
	yslopetab = (fixed_t*)Z_Malloc(sizeof(fixed_t) * (vid.height * 4), PU_STATIC, NULL);
	
	cachedheight = (fixed_t*)Z_Malloc(sizeof(fixed_t) * vid.height, PU_STATIC, NULL);
	cacheddistance = (fixed_t*)Z_Malloc(sizeof(fixed_t) * vid.height, PU_STATIC, NULL);
	cachedxstep = (fixed_t*)Z_Malloc(sizeof(fixed_t) * vid.height, PU_STATIC, NULL);
	cachedystep = (fixed_t*)Z_Malloc(sizeof(fixed_t) * vid.height, PU_STATIC, NULL);
	spanstart = (int*)Z_Malloc(sizeof(int) * vid.height, PU_STATIC, NULL);
	...
	if (drawsegs)
		for (i = 0; i < maxdrawsegs; i++)
		{
			...
			drawsegs[i].frontscale = (fixed_t*)Z_Malloc(sizeof(fixed_t) * MAXSEGS, PU_STATIC, NULL);
		}
		...
		for (i = 0; i < MAXVISPLANES; i++)
			if (visplane_ptr[i])
				for (pl = visplane_ptr[i]; pl; pl = pl->next)
				{
					...
					pl->top = (uint16_t*)Z_Malloc(sizeof(unsigned short) * (vid.width + 2), PU_STATIC, NULL);
					pl->top = &pl->top[1];
					...
					pl->bottom = (uint16_t*)Z_Malloc(sizeof(unsigned short) * (vid.width + 2), PU_STATIC, NULL);
	...
	for (i = 0; i < MAXFFLOORS; i++)
	{
		...
		ffloor[i].f_clip = (short int*)Z_Malloc(sizeof(short) * vid.width, PU_STATIC, NULL);
		
		if (ffloor[i].c_clip)
			SCRFREE(ffloor[i].c_clip);
		ffloor[i].c_clip = (short int*)Z_Malloc(sizeof(short) * vid.width, PU_STATIC, NULL);
	}
}
