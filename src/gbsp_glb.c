...
static void AddExtraFile(nodebuildinfo_t * info, const char *str)
{
	...
	if (!info->extra_files)
	{
		info->extra_files = (const char **)UtilCalloc(EXTRA_BLOCK * sizeof(const char *));
	...
	if (space == 0)
	{
		info->extra_files = (const char **)UtilRealloc((void *)info->extra_files, (count + 1 + EXTRA_BLOCK) * sizeof(const char *));
	}
...
glbsp_ret_e GlbspParseArgs(nodebuildinfo_t * info, volatile nodebuildcomms_t * comms, const char **argv, int argc)
{
	const char *opt_str;
	int num_files = 0;
	...
	while(argc > 0)
	{
		...
		if (UtilStrCaseCmp(opt_str, "factor") == 0 || UtilStrCaseCmp(opt_str, "c") == 0)
		{
			if (argc < 2)
			{
		...
		if (UtilStrCaseCmp(opt_str, "maxblock") == 0 || UtilStrCaseCmp(opt_str, "b") == 0)
		{
			if (argc < 2)
			{
		...
		HANDLE_BOOLEAN2("q", "quiet", quiet)
			...
			HANDLE_BOOLEAN2("s", "skipselfref", skip_self_ref) HANDLE_BOOLEAN2("xu", "noprune", no_prune) HANDLE_BOOLEAN2("xn", "nonormal", no_normal)
			// to err is human...
			HANDLE_BOOLEAN("noprogress", no_progress) HANDLE_BOOLEAN("packsides", pack_sides) HANDLE_BOOLEAN("prunesect", prune_sect)
			// ignore these options for backwards compatibility
			if (UtilStrCaseCmp(opt_str, "fresh") == 0 ||
				UtilStrCaseCmp(opt_str, "keepdummy") == 0 || UtilStrCaseCmp(opt_str, "keepsec") == 0 || UtilStrCaseCmp(opt_str, "keepsect") == 0)
		{
			argv++;
			argc--;
			continue;
		...
		HANDLE_BOOLEAN("loadall", load_all)
			...
			HANDLE_BOOLEAN("hexen", force_hexen) SetErrorMsg("Unknown option: %s", argv[0]);
...
{
	glbsp_superblock_t *seg_list;
	glbsp_glbsp_node_t *root_node;
	glbsp_glbsp_node_t *root_stale_node;
	glbsp_subsec_t *root_sub;
...
glbsp_ret_e GlbspBuildNodes(const nodebuildinfo_t * info, const nodebuildfuncs_t * funcs, volatile nodebuildcomms_t * comms)
{
	char *file_msg;
	...
	// sanity check
	if (!cur_info->input_file || cur_info->input_file[0] == 0 || !cur_info->output_file || cur_info->output_file[0] == 0)
	{
