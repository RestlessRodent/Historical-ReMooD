...
bool EV_HExDoGenElevator(line_t* const a_Line, mobj_t* const a_Object)
{
	EV_GenHESpeed_t HEFSpeed, HEFDoorSpeed;
	EV_GenHEElevType_t HEFType;
	EV_GenHEFCDWait_t HEFWait;
	bool HEFSilent, HEFDoor;
	bool Ret, StnMove, IsPerp;
	int SectorNum;
	sector_t* CurSec;
	...
	HEFSpeed = (EV_GenHESpeed_t)((a_Line->special & EVGENGE_SPEEDMASK) >> EVGENGE_SPEEDSHIFT);
	HEFType = (EV_GenHEElevType_t)((a_Line->special & EVGENGE_ELEVTYPEMASK) >> EVGENGE_ELEVTYPESHIFT);
	HEFWait = (EV_GenHEFCDWait_t)((a_Line->special & EVGENGE_ELEVWAITMASK) >> EVGENGE_ELEVWAITSHIFT);
	HEFDoorSpeed = (EV_GenHESpeed_t)((a_Line->special & EVGENGE_ELEVDSPEEDMASK) >> EVGENGE_ELEVDSPEEDSHIFT);
	...
	while ((SectorNum = P_FindSectorFromLineTag(a_Line, SectorNum)) >= 0)
	{
		...
		if (CurSec->floordata || CurSec->ceilingdata)
		{
			...
			ElevThinker = (elevator_t*)CurSec->floordata;
			...
			ElevThinker = (elevator_t*)CurSec->ceilingdata;
			if (ElevThinker && ElevThinker->thinker.function.acp1 == (actionf_p1)T_MoveElevator)
				if (ElevThinker->type == EVGHEELEVT_PERPUP || ElevThinker->type == EVGHEELEVT_PERPDOWN)
					IsPerp = true;
			...
			if (IsPerp && (HEFType == EVGHEELEVT_CALLPERP || HEFType == EVGHEELEVT_STOPPERP))
			{
				...
				else
				{
					...
					if (a_Line->frontsector->floorheight != ElevThinker->sector->floorheight)
					{
						...
						S_StartSound((S_NoiseThinker_t*)&a_Line->backsector->soundorg, sfx_elvcal);
					}
				}
		...
		ElevThinker = (elevator_t*)Z_Malloc(sizeof(*ElevThinker), PU_LEVSPEC, NULL);
...
bool EV_HExDoGenPlat(line_t* const a_Line, mobj_t* const a_Object)
{
	plat_t* NewPlat;
	...
	/* Extract Variables */
	HEFSpeed = (EV_GenHESpeed_t)((a_Line->special & EVGENGE_SPEEDMASK) >> EVGENGE_SPEEDSHIFT);
	HEFType = (EV_GenHEFType_t)((a_Line->special & EVGENGE_FCPTYPEMASK) >> EVGENGE_FCPTYPESHIFT);
	HEFWait = (EV_GenHEFCDWait_t)((a_Line->special & EVGENGE_FCDWAITMASK) >> EVGENGE_FCDWAITSHIFT);
	HEFMode = (floorchange_e)((a_Line->special & EVGENGE_FCPMODEMASK) >> EVGENGE_FCPMODESHIFT);
	...
	while ((SectorNum = P_FindSectorFromLineTag(a_Line, SectorNum)) >= 0)
	{
		...
		Ret = true;
		NewPlat = (plat_t*)Z_Malloc(sizeof(*NewPlat), PU_LEVSPEC, 0);
		...
		if (HEFType == EVGHEPLATT_LHFPERP)
		{
			...
			// Randomize up/down
			NewPlat->status = (plat_e)(P_Random() & 1);
...
bool EV_TryGenTrigger(line_t* const a_Line, const int a_Side, mobj_t* const a_Object, const EV_TryGenType_t a_Type, const uint32_t a_Flags, bool* const a_UseAgain)
{
	...
	if (a_Line->special >= 0x2F80 && a_Line->special <= 0x7FFF)
	{
		...
		TrigMode = (triggertype_e)((a_Line->special & TriggerType) >> TriggerTypeShift);
		...
		// Lose that bit
		TrigMode = (triggertype_e)((int)TrigMode & (~1));
	...
	else if (a_Line->special > 0xFFFF)
	{
		...
		{
			// Extract Trigger
			TrigMode = (triggertype_e)((a_Line->special & EVGENGE_TRIGMASK) >> EVGENGE_TRIGSHIFT);
...
{
	bool REx, EffectMonster, IsMonsterMo, DoDam;
	uint32_t Bits;
	...
	else
	{
		...
		if (a_Player && (a_Sector->special & SECRET_MASK))
		{
			...
			if (!P_EXGSGetValue(PEXGSBID_GAMEDEATHMATCH))
				P_PlayerMessage(PPM_SECRET, a_Mo, NULL, (const char**)PTROFUNICODESTRING(DSTR_FOUNDSECRET));
		}
