...
bool M_ExMenuHandleEvent(const I_EventEx_t* const a_Event)
{
...
void M_ExMenuDrawer(void)
{
	...
	for (i = 0; i < __REMOOD_NUMSPLITS; i++)
	{
		...
		ya = V_FontHeight((VideoFont_t)l_MenuFont.Value[0].Int) + 2;
		...
		for (j = TopMenu->StartOff; j < UI->NumItems && y < ((ScrY + ScrH) - 20); j++, y += ya)
		{
			...
			if (j == TopMenu->CurItem)
				if (g_ProgramTic & 0x4)
					V_DrawCharacterMB(
						(VideoFont_t)l_MenuFont.Value[0].Int,
			...
			if (Item->Value)
			{
				...
				V_StringDimensionsA((VideoFont_t)l_MenuFont.Value[0].Int, 0, ValStr, &ValW, &ValH);
...
M_UIMenuHandler_t* M_ExPushMenu(const uint8_t a_Player, M_UIMenu_t* const a_UI)
{
	...
	/* Allocate */
	New = (M_UIMenuHandler_t*)Z_Malloc(sizeof(*New), PU_STATIC, NULL);
...
#define UIMIDSPACE (UISPACING >> 1)
	...
	New = l_UIBoxes[l_NumUIBoxes++] = (M_UILocalBox_t*)Z_Malloc(sizeof(M_UILocalBox_t), PU_STATIC, NULL);
