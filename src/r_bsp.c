...
void R_Subsector(int num)
{
											  ...
											  ceilingpic == skyflatnum)))
	{
		floorplane = R_FindPlane(frontsector->floorheight,
								 ...
								 frontsector->floor_yoffs, floorcolormap, NULL);
	}
	else
		floorplane = NULL;
		 ...
		 (frontsector->heightsec != -1 && sectors[frontsector->heightsec].floorpic == skyflatnum)))
	{
		ceilingplane = R_FindPlane(frontsector->ceilingheight,
								   ...
								   frontsector->ceiling_yoffs, ceilingcolormap, NULL);
	}
	else
		ceilingplane = NULL;
	...
	if (frontsector->ffloors)
	{
		...
		for (rover = frontsector->ffloors; rover && numffloors < MAXFFLOORS; rover = rover->next)
		{
			...
			{
				light =
					R_GetPlaneLight(frontsector, *rover->bottomheight,
									viewz < *rover->bottomheight ? true : false);
				ffloor[numffloors].plane =
					R_FindPlane(*rover->bottomheight, *rover->bottompic,
								*frontsector->lightlist[light].lightlevel,
								*rover->bottomxoffs, *rover->bottomyoffs,
			...
			{
				light =
					R_GetPlaneLight(frontsector, *rover->topheight,
									viewz < *rover->topheight ? true : false);
				ffloor[numffloors].plane =
					R_FindPlane(*rover->topheight, *rover->toppic,
								*frontsector->lightlist[light].lightlevel,
								*rover->topxoffs, *rover->topyoffs,
								frontsector->lightlist[light].extra_colormap, rover);
...
void R_Prep3DFloors(sector_t * sector)
{
	...
	for (rover = sector->ffloors; rover; rover = rover->next)
	{
		if ((rover->flags & FF_EXISTS) &&
			(!(rover->flags & FF_NOSHADE) || (rover->flags & FF_CUTLEVEL) ||
