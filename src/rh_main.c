...
extern bool_t setsizeneeded;					// Need screen resize?
...
void RH_InitBuffers(RH_RenderStat_t* const a_Stat)
{
	...
	if (Info->AllocWidth != a_Stat->viewwidth || Info->AllocHeight != a_Stat->viewheight)
	{
		...
		// Allocation
		Info->columnofs = Z_Malloc(sizeof(*Info->columnofs) * a_Stat->viewwidth, PU_STATIC, NULL);
		Info->ylookup = Z_Malloc(sizeof(*Info->ylookup) * a_Stat->viewheight, PU_STATIC, NULL);
	...
	if (Info->VidW != vid.width || Info->VidH != vid.height)
	{
		...
		// Allocation
		Info->yslopetab = Z_Malloc(sizeof(*Info->yslopetab) * (vid.height * 4), PU_STATIC, NULL);
		Info->spanstart = Z_Malloc(sizeof(*Info->spanstart) * vid.height, PU_STATIC, NULL);
		Info->spanstop = Z_Malloc(sizeof(*Info->spanstop) * vid.height, PU_STATIC, NULL);
		Info->cachedheight = Z_Malloc(sizeof(*Info->cachedheight) * vid.height, PU_STATIC, NULL);
		Info->cacheddistance = Z_Malloc(sizeof(*Info->cacheddistance) * vid.height, PU_STATIC, NULL);
		Info->cachedxstep = Z_Malloc(sizeof(*Info->cachedxstep) * vid.height, PU_STATIC, NULL);
		Info->cachedystep = Z_Malloc(sizeof(*Info->cachedystep) * vid.height, PU_STATIC, NULL);
		Info->distscale = Z_Malloc(sizeof(*Info->distscale) * vid.width, PU_STATIC, NULL);
		Info->xtoviewangle = Z_Malloc(sizeof(*Info->xtoviewangle) * (vid.width + 1), PU_STATIC, NULL);
		Info->viewangletox = Z_Malloc(sizeof(*Info->viewangletox) * (FINEANGLES / 2), PU_STATIC, NULL);
