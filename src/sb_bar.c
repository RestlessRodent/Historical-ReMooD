...
void SB_Init(void)
{
	...
	{
		PatchLIFEGEM = W_CachePatchNum(W_GetNumForName("LIFEGEM0") + consoleplayer[0], PU_STATIC);
	}
...
void SB_Ticker(void)
{
	...
	curHealth = players[consoleplayer[0]].mo->health;
	if (curHealth < 0)
	{
		curHealth = 0;
	}
...
static void DrINumber(signed int val, int x, int y)
{
	...
	if (val < 0)
	{
		if (val < -9)
		{
			V_DrawScaledPatch(x + 1, y + 1, FG, W_CachePatchName("LAME", PU_CACHE));
		}
		else
		{
			val = -val;
			V_DrawScaledPatch(x + 18, y, FG, PatchINumbers[val]);
			V_DrawScaledPatch(x + 9, y, FG, PatchNEGATIVE);
		}
		return;
	}
	if (val > 99)
	{
		...
		V_DrawScaledPatch(x, y, FG, patch);
	}
	val = val % 100;
	if (val > 9 || oldval > 99)
	{
		patch = PatchINumbers[val / 10];
		V_DrawScaledPatch(x + 9, y, FG, patch);
	}
	val = val % 10;
	patch = PatchINumbers[val];
	V_DrawScaledPatch(x + 18, y, FG, patch);
}
...
//    V_DrawTranslucentPatch(x+2,y+2,V_SCALESTART|0,p/*,transtables*/);
	V_DrawScaledPatch(x, y, V_SCALESTART, p);
}
...
static void DrSmallNumber(int val, int x, int y)
{
	...
	V_DrawScaledPatch(x + 4, y, FG, patch);
}
...
static void ShadeChain(void)
{
	...
	for (i = 0; i < 16 * st_scalex; i++)
	{
		...
		ShadeLine((st_x + 19) * st_scalex + i, (ST_Y + 32) * st_scaley, 10 * st_scaley,
				  7 - (i / 4));
	}
}
...
void SB_Drawer(boolean refresh)
{
	...
	CPlayer = &players[displayplayer[0]];
	if (!st_statusbaron)
	{
	...
	else
	{
		if (SB_state == -1)
		{
			V_CopyRect(0, vid.height - stbarheight, BG, vid.width, stbarheight, 0,
						   vid.height - stbarheight, FG);

			V_DrawScaledPatch(st_x, ST_Y, FG, PatchBARBACK);
			if (players[consoleplayer[0]].cheats & CF_GODMODE)
			{
				V_DrawScaledPatch(st_x + 16, ST_Y + 9, FG,
								  W_CachePatchName("GOD1", PU_CACHE));
				V_DrawScaledPatch(st_x + 287, ST_Y + 9, FG,
								  W_CachePatchName("GOD2", PU_CACHE));
			}
			oldhealth = -1;
		}
		DrawCommonBar();
		if (!CPlayer->st_inventoryTics)
		{
			...
			{
				// Main interface
				V_DrawScaledPatch(st_x + 34, ST_Y + 2, FG, PatchSTATBAR);
				oldarti = -1;
				oldammo = -1;
				oldarmor = -1;
				oldweapon = -1;
		...
		else
		{
			if (SB_state != 1)
			{
				V_DrawScaledPatch(st_x + 34, ST_Y + 2, FG, PatchINVBAR);
			}
			DrawInventoryBar();
			SB_state = 1;
		}
	...
	{
		if (CPlayer->powers[pw_flight] > BLINKTHRESHOLD || !(CPlayer->powers[pw_flight] & 16))
		{
			...
			if (CPlayer->mo->flags2 & MF2_FLY)
			{
				...
				{
					V_DrawScaledPatch(20, 17, FG, W_CachePatchNum(spinflylump + 15, PU_CACHE));
				}
				else
				{
					V_DrawScaledPatch(20, 17, FG, W_CachePatchNum(spinflylump + frame, PU_CACHE));
...
void SB_PaletteFlash(void)
{
	...
	CPlayer = &players[consoleplayer[0]];
...
static void DrawCommonBar(void)
{
	...
	V_DrawScaledPatch(st_x, ST_Y - 10, FG, PatchLTFCTOP);
	V_DrawScaledPatch(st_x + 290, ST_Y - 10, FG, PatchRTFCTOP);
	...
	if (oldhealth != HealthMarker)
	{
		...
		V_DrawScaledPatch(st_x, ST_Y + 32, FG, PatchCHAINBACK);
		V_DrawScaledPatch(st_x + 2 + (healthPos % 17), ST_Y + 33 + chainY, FG, PatchCHAIN);
		V_DrawScaledPatch(st_x + 17 + healthPos, ST_Y + 33 + chainY, FG, PatchLIFEGEM);
		V_DrawScaledPatch(st_x, ST_Y + 32, FG, PatchLTFACE);
		V_DrawScaledPatch(st_x + 276, ST_Y + 32, FG, PatchRTFACE);
		ShadeChain();
...
static void DrawMainBar(void)
{
	...
	if (ArtifactFlash)
	{
		V_DrawScaledPatch(st_x + 180, ST_Y + 3, FG, PatchBLACKSQ);
		V_DrawScaledPatch(st_x + 182, ST_Y + 3, FG,
						  W_CachePatchNum(W_GetNumForName("useartia") + ArtifactFlash - 1,
										  PU_CACHE));
		ArtifactFlash--;
			 ...
			 || oldartiCount != CPlayer->inventory[CPlayer->inv_ptr].count)
	{
		...
		{
			V_DrawScaledPatch(st_x + 179, ST_Y + 2, FG,
							  W_CachePatchName(patcharti[CPlayer->inventory[CPlayer->inv_ptr].type],
											   PU_CACHE));
	...
	if (cv_deathmatch.value)
	{
		...
		if (temp != oldfrags)
		{
			V_DrawScaledPatch(st_x + 57, ST_Y + 13, FG, PatchARMCLEAR);
	...
	{
		if (CPlayer->cards & it_hyellowkey)
		{
			V_DrawScaledPatch(st_x + 153, ST_Y + 6, FG,
							  W_CachePatchName("ykeyicon", PU_CACHE));
		}
		if (CPlayer->cards & it_hgreenkey)
		{
			V_DrawScaledPatch(st_x + 153, ST_Y + 14, FG,
							  W_CachePatchName("gkeyicon", PU_CACHE));
		}
		if (CPlayer->cards & it_hbluekey)
		{
			V_DrawScaledPatch(st_x + 153, ST_Y + 22, FG,
							  W_CachePatchName("bkeyicon", PU_CACHE));
		}
		oldkeys = playerkeys;
		UpdateState |= I_STATBAR;
	}
	...
	{
		V_DrawScaledPatch(st_x + 108, ST_Y + 3, FG, PatchBLACKSQ);
		if (temp && CPlayer->readyweapon > 0 && CPlayer->readyweapon < 7)
		{
			DrINumber(temp, st_x + 109, ST_Y + 4);
			V_DrawScaledPatch(st_x + 111, ST_Y + 14, FG,
							  W_CachePatchName(ammopic[CPlayer->readyweapon - 1], PU_CACHE));
		}
		oldammo = temp;
	...
	{
		V_DrawScaledPatch(st_x + 224, ST_Y + 13, FG, PatchARMCLEAR);
...
static void DrawInventoryBar(void)
{
	...
	for (i = 0; i < 7; i++)
	{
		...
		{
			V_DrawScaledPatch(st_x + 50 + i * 31, ST_Y + 2, FG,
							  W_CachePatchName(patcharti[CPlayer->inventory[x + i].type],
											   PU_CACHE));
			DrSmallNumber(CPlayer->inventory[x + i].count, st_x + 69 + i * 31, ST_Y + 24);
		}
	}
	V_DrawScaledPatch(st_x + 50 + CPlayer->st_curpos * 31, ST_Y + 31, FG, PatchSELECTBOX);
	if (x != 0)
	{
		V_DrawScaledPatch(st_x + 38, ST_Y + 1, FG, !(leveltime & 4) ? PatchINVLFGEM1 :
						  PatchINVLFGEM2);
	}
	if (CPlayer->inventorySlotNum - x > 7)
	{
		V_DrawScaledPatch(st_x + 269, ST_Y + 1, FG, !(leveltime & 4) ?
...
static void DrawFullScreenStuff(void)
{
	...
	if (!CPlayer->st_inventoryTics)
	{
		if (CPlayer->inventory[CPlayer->inv_ptr].type > 0)
		{
			V_DrawFuzzPatch(st_x + 286, ST_Y + 12, W_CachePatchName("ARTIBOX", PU_CACHE));
			V_DrawScaledPatch(st_x + 286, ST_Y + 12, FG,
							  W_CachePatchName(patcharti[CPlayer->inventory[CPlayer->inv_ptr].type],
											   PU_CACHE));
	...
	else
	{
		...
		for (i = 0; i < 7; i++)
		{
			V_DrawFuzzPatch(st_x + 50 + i * 31, ST_Y + 10, W_CachePatchName("ARTIBOX", PU_CACHE));
			if (CPlayer->inventorySlotNum > x + i && CPlayer->inventory[x + i].type != arti_none)
			{
				V_DrawScaledPatch(st_x + 50 + i * 31, ST_Y + 10, FG,
								  W_CachePatchName(patcharti[CPlayer->inventory[x + i].type],
												   PU_CACHE));
				DrSmallNumber(CPlayer->inventory[x + i].count, 69 + i * 31, ST_Y + 32);
			}
		}
		V_DrawScaledPatch(st_x + 50 + CPlayer->st_curpos * 31, ST_Y + 39, FG, PatchSELECTBOX);
		if (x != 0)
		{
			V_DrawScaledPatch(st_x + 38, ST_Y + 9, FG, !(leveltime & 4) ? PatchINVLFGEM1 :
							  PatchINVLFGEM2);
		}
		if (CPlayer->inventorySlotNum - x > 7)
		{
			V_DrawScaledPatch(st_x + 269, ST_Y + 9, FG, !(leveltime & 4) ?
							  PatchINVRTGEM1 : PatchINVRTGEM2);
		}
	}
}
