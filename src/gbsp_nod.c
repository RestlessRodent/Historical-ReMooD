...
static glbsp_superblock_t *quick_alloc_supers = NULL;
...
// NewSuperBlock
//
static glbsp_superblock_t *NewSuperBlock(void)
{
	glbsp_superblock_t *block;

	if (quick_alloc_supers == NULL)
		return UtilCalloc(sizeof(glbsp_superblock_t));

	block = quick_alloc_supers;
	quick_alloc_supers = block->subs[0];

	// clear out any old rubbish
	memset(block, 0, sizeof(glbsp_superblock_t));
...
void FreeQuickAllocSupers(void)
{
	...
	{
		glbsp_superblock_t *block = quick_alloc_supers;
...
static glbsp_seg_t *CreateOneSeg(glbsp_linedef_t * line, glbsp_vertex_t * start, glbsp_vertex_t * end, glbsp_sidedef_t * side, int side_num)
{
	glbsp_seg_t *seg = NewSeg();
...
glbsp_superblock_t *CreateSegs(void)
{
	...
	{
		glbsp_linedef_t *line = LookupLinedef(i);
		...
		if (ABS(line->start->x - line->end->x) >= 10000 || ABS(line->start->y - line->end->y) >= 10000)
		{
...
// DetermineMiddle
//
static void DetermineMiddle(glbsp_subsec_t * sub)
{
	glbsp_seg_t *cur;
...
static void ClockwiseOrder(glbsp_subsec_t * sub)
{
	...
	if (total <= 32)
		...
		array = UtilCalloc(total * sizeof(glbsp_seg_t *));
...
// SanityCheckClosed
//
static void SanityCheckClosed(glbsp_subsec_t * sub)
{
	glbsp_seg_t *cur, *next;
	int total = 0, gaps = 0;
...
//
static void SanityCheckSameSector(glbsp_subsec_t * sub)
{
	glbsp_seg_t *cur;
	glbsp_seg_t *compare;
	...
	for (cur = compare->next; cur; cur = cur->next)
	{
		...
		if (cur->linedef)
			PrintMiniWarn("Sector #%d has sidedef facing #%d (line #%d) "
						  "near (%1.0f,%1.0f).\n", compare->sector->index, cur->sector->index, cur->linedef->index, sub->mid_x, sub->mid_y);
		else
			PrintMiniWarn("Sector #%d has sidedef facing #%d " "near (%1.0f,%1.0f).\n", compare->sector->index, cur->sector->index, sub->mid_x, sub->mid_y);
	}
}
...
//
static void SanityCheckHasRealSeg(glbsp_subsec_t * sub)
{
	glbsp_seg_t *cur;
	...
	InternalError("Subsector #%d near (%1.1f,%1.1f) has no real seg !", sub->index, sub->mid_x, sub->mid_y);
}

//
// RenumberSubsecSegs
//
static void RenumberSubsecSegs(glbsp_subsec_t * sub)
{
	glbsp_seg_t *cur;
...
static void CreateSubsecWorker(glbsp_subsec_t * sub, glbsp_superblock_t * block)
{
	int num;
	...
	for (num = 0; num < 2; num++)
	{
		glbsp_superblock_t *A = block->subs[num];
...
static void DebugShowSegs(glbsp_superblock_t * seg_list)
{
	glbsp_seg_t *cur;
	int num;
...
{
	glbsp_glbsp_node_t *node;
	glbsp_seg_t *best;

	glbsp_superblock_t *rights;
	glbsp_superblock_t *lefts;

	glbsp_intersection_t *cut_list;
	...
	*S = NULL;
	...
	lefts = (glbsp_superblock_t *) NewSuperBlock();
	rights = (glbsp_superblock_t *) NewSuperBlock();
	...
	*N = node = NewNode();
	...
	if (best->p_length >= 30000)
	{
		...
		{
			PrintMiniWarn("Loss of accuracy on VERY long node: " "(%d,%d) -> (%d,%d)\n", node->x, node->y, node->x + node->dx, node->y + node->dy);
		}
	...
	ret = BuildNodes(lefts, &node->l.node, &node->l.subsec, depth + 1, stale_nd ? (stale_opposite ? stale_nd->r.node : stale_nd->l.node) : NULL);
	FreeSuper(lefts);
	...
	ret = BuildNodes(rights, &node->r.node, &node->r.subsec, depth + 1, stale_nd ? (stale_opposite ? stale_nd->l.node : stale_nd->r.node) : NULL);
	FreeSuper(rights);
...
void ClockwiseBspTree(glbsp_glbsp_node_t * root)
{
	...
	{
		glbsp_subsec_t *sub = LookupSubsec(i);
