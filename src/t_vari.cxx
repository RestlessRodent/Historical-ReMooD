...
svariable_t* new_variable(script_t* script, char* name, int vtype)
{
	int n;
	svariable_t* newvar;
	int tagtype = script == &global_script || script == &hub_script ? PU_STATIC : PU_LEVEL;
	
	// find an empty slot first
	
	newvar = (svariable_t*)Z_Malloc(sizeof(svariable_t), (Z_MemoryTag_t)tagtype, 0);
	newvar->name = (char*)Z_Strdup(name, (Z_MemoryTag_t)tagtype, 0);
	newvar->type = vtype;
	
	if (vtype == svt_string)
	{
		// 256 bytes for string
		newvar->value.s = (char*)Z_Malloc(256, (Z_MemoryTag_t)tagtype, 0);
		newvar->value.s[0] = 0;
	}
	else
		newvar->value.i = 0;
...
{
	if (killscript)
		return;					// protect the variables when killing script
...
svalue_t evaluate_function(int start, int stop)
{
	...
	if (tokentype[start] != function || tokentype[stop] != fragglescript_operator || tokens[stop][0] != ')')
	...
	if (killscript)
		return nullvar;			// one of the above errors occurred
...
svariable_t* new_function(const char* name, void (*handler) ())
{
	...
	newvar = new_variable(&global_script, (char*)name, svt_function);
