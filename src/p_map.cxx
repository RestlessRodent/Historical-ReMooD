...
// PIT_StompThing
//
static bool PIT_StompThing(mobj_t* thing, void* a_Arg)
{
	fixed_t blockdist;
...
{
	bool VsInfo;								// Use against mobjinfo, not mobj_t
} P_PITCTSettings_t;

/* PIT_CheckThing() -- Check thing */
static bool PIT_CheckThing(mobj_t* thing, void* a_Arg)
{
	fixed_t blockdist;
	bool solid, Dammed;
	int damage;
	P_PITCTSettings_t* SettingsP = (P_PITCTSettings_t*)a_Arg;
...
//
static bool PIT_CrossLine(line_t* ld, void* a_Arg)
{
...
//
bool PIT_CheckLine(line_t* ld, void* a_Arg)
{
...
static bool PIT_CheckThingDet(mobj_t* thing, void* a_Arg)
{
	fixed_t blockdist;
	bool solid;
	int damage;
...
bool PIT_CheckLineDet(line_t* ld, void* a_Arg)
{
...
static bool PS_SubTryMove(mobj_t* thing, fixed_t x, fixed_t y, bool allowdropoff, const uint32_t a_Flags)
{
	fixed_t oldx;
	fixed_t oldy;
	int side;
...
bool PTR_ShootTraverse(intercept_t* in, void* a_Data)
{
	...
	P_LineAtkArgs_t* Args = (P_LineAtkArgs_t*)a_Data;
...
//
bool PIT_RadiusAttack(mobj_t* thing, void* a_Arg)
{
	fixed_t dx;
	fixed_t dy;
	fixed_t dz;
...
bool PIT_CheckRadius(mobj_t* thing, void* a_Arg)
{
	fixed_t blockdist;
	bool solid;
	int damage;
...
bool PIT_CheckRadiusLine(line_t* ld, void* a_Arg)
{
	if (tmbbox[BOXRIGHT] <= ld->bbox[BOXLEFT]
	        || tmbbox[BOXLEFT] >= ld->bbox[BOXRIGHT] || tmbbox[BOXTOP] <= ld->bbox[BOXBOTTOM] || tmbbox[BOXBOTTOM] >= ld->bbox[BOXTOP])
		return true;
...
// PIT_ChangeSector
//
bool PIT_ChangeSector(mobj_t* thing, void* a_Arg)
{
	mobj_t* mo;
	mobjtype_t GibsType;
...
bool PIT_GetSectors(line_t* ld, void* a_Arg)
{
	if (tmbbox[BOXRIGHT] <= ld->bbox[BOXLEFT] ||
	        tmbbox[BOXLEFT] >= ld->bbox[BOXRIGHT] || tmbbox[BOXTOP] <= ld->bbox[BOXBOTTOM] || tmbbox[BOXBOTTOM] >= ld->bbox[BOXTOP])
		return true;
...
static bool PIT_CheckOnmobjZ(mobj_t* thing, void* a_Arg)
{
	fixed_t blockdist;
