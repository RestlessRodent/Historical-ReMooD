...
char* Z_StrDupWrappee(const char* const String, const Z_MemoryTag_t Tag, void** Ref _ZMGD_WRAPPEE)
{
	...
	n = strlen(String);
#if defined(_DEBUG)
	Ptr = (char*)Z_MallocExWrappee(sizeof(char) * (n + 1), Tag, Ref, 0, File, -Line);
#else
	Ptr = (char*)Z_Malloc(sizeof(char) * (n + 1), Tag, Ref);
#endif
	strcpy(Ptr, String);
...
struct Z_HashTable_s
{
	...
	bool (*CompareFunc) (void* const a_A, void* const a_B);
};
...
Z_HashTable_t* Z_HashCreateTable(bool (*a_CompareFunc) (void* const a_A, void* const a_B))
{
	...
	New = (Z_HashTable_t*)Z_Malloc(sizeof(*New), PU_STATIC, NULL);
...
static bool Z_TableCompareFunc(void* const a_A, void* const a_B)
{
	Z_TableEntry_t* B;
	...
	/* Get B */
	B = (Z_TableEntry_t*)a_B;
	
	/* Is a table? */
	if (B->IsTable)
	{
		if (strcasecmp((const char*)a_A, B->Data.TableLink->Key) == 0)
			return true;
	}
	...
	{
		if (strcasecmp((const char*)a_A, B->Data.Index.SubKey) == 0)
			return true;
	}
...
Z_Table_t* Z_TableCreate(const char* const a_Key)
{
	...
	/* Allocate */
	NewTable = (Z_Table_t*)Z_Malloc(sizeof(*NewTable), PU_STATIC, NULL);
...
static Z_TableEntry_t* Z_PushNewEntry(Z_Table_t* const a_Table, const char* const a_Key)
{
	...
	a_Table->Entries[Num] = (Z_TableEntry_t*)Z_Malloc(sizeof(*a_Table->Entries[Num]), PU_STATIC, NULL);
...
static Z_TableEntry_t* Z_FindTableEntry(Z_Table_t* const a_Table, const char* const a_Key, const bool a_Create, bool* const a_Created)
{
	uint32_t Hash;
	...
	/* Find actual entry */
	Entry = (Z_TableEntry_t*)Z_HashFindEntry(a_Table->EntryHashes, Hash, (void*)a_Key, false);
