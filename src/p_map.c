...
static boolean PIT_CheckThing(mobj_t * thing)
{
	...
	if (tmthing->flags & MF_MISSILE)
	{
		...
		if (P_DamageMobj(thing, tmthing, tmthing->target, damage) &&
			...
			P_SpawnBloodSplats(tmthing->x, tmthing->y, tmthing->z, damage,
...
boolean P_TryMove(mobj_t * thing, fixed_t x, fixed_t y, boolean allowdropoff)
{
	...
	if (!(thing->flags & MF_NOCLIP))
	{
		...
		if (!boomsupport || !allowdropoff)
			if (!(thing->flags & (MF_DROPOFF | MF_FLOAT))
				&& !tmfloorthing && tmfloorz - tmdropoffz > MAXSTEPMOVE)
	...
	if (demoversion >= 112 && demoversion < 128 && thing->player &&
