...
static bool MS_TextureTestUnderDraw(struct M_UIMenuHandler_s* const a_Handler, struct M_UIMenu_s* const a_Menu, const int32_t a_X, const int32_t a_Y, const int32_t a_W, const int32_t a_H)
{
#define BUFSIZE 32
	texture_t* Texture;
	char Buf[BUFSIZE];
	...
	if (Texture->IsFlat)
	{
		if (R_GetFlat(a_Menu->Items[a_Handler->CurItem].DataBits))
			V_ImageDraw(0, (V_Image_t*)Texture->FlatImage, a_X + (a_W >> 1), a_Y + ((a_H - Texture->height) - 5), NULL);
	}
...
CONL_ExitCode_t M_ExGeneralComm(const uint32_t a_ArgC, const char** const a_ArgV)
{
	...
	{
		// Allocate
		New = (M_UIMenu_t*)Z_Malloc(sizeof(*New), PU_STATIC, NULL);
	
		// Quick
		New->NumItems = numtextures;
		New->Items = (M_UIItem_t*)Z_Malloc(sizeof(*New->Items) * New->NumItems, PU_STATIC, NULL);
...
static bool MS_GameVar_LRValChange(struct M_UIMenu_s* const a_Menu, struct M_UIItem_s* const a_Item, const bool a_More)
{
	P_EXGSVariable_t* Bit;
	int32_t OldVal, NewVal, ModVal;
	
	/* Get bit */
	Bit = P_EXGSVarForBit((P_EXGSBitID_t)a_Item->DataBits);
...
M_UIMenu_t* M_ExTemplateMakeGameVars(const int32_t a_Mode)
{
	...
	/* Sort bits */
	SortedBits = (P_EXGSVariable_t**)Z_Malloc(sizeof(*SortedBits) * (PEXGSNUMBITIDS - 1), PU_STATIC, NULL);
	
	// Init
	for (i = 1; i < PEXGSNUMBITIDS; i++)
		SortedBits[i - 1] = P_EXGSVarForBit((P_EXGSBitID_t)i);
	...
	// Sort by name now
	LastCat = (P_EXGSMenuCategory_t)0;
	...
	for (j = 0, i = 0; i < New->NumItems; i++)
	{
		...
		if (LastCat != Bit->Category)
		{
			...
			New->Items[i].TextRef = (const char**)PTROFUNICODESTRING((DSTR_MENUGAMEVAR_CATNONE + LastCat));
