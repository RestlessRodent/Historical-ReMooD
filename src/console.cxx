...
static bool l_CONLActive = false;	// Console active?
...
CONCTI_Inputter_t* CONCTI_CreateInput(const size_t a_NumHistory, const CONCTI_OutBack_t a_OutBack, CONCTI_Inputter_t** const a_RefPtr)
{
	...
	/* Create new */
	New = (CONCTI_Inputter_t*)Z_Malloc(sizeof(*New), PU_STATIC, NULL);
	...
	if (New->NumHistory)
		New->History = (char**)Z_Malloc(sizeof(*New->History) * New->NumHistory, PU_STATIC, NULL);
...
#define BUFSIZE 512
	...
	switch (Code)
	{
		...
		default:
			...
			if (!MBRover)
			{
				MBRover = a_Input->ChainRoot = (CONCTI_MBChain_t*)Z_Malloc(sizeof(*MBRover), PU_STATIC, NULL);
			...
			else
			{
				...
				if (!MBRover)
				{
					MBRover = Last->Next = (CONCTI_MBChain_s*)Z_Malloc(sizeof(*MBRover), PU_STATIC, NULL);
				...
				{
					Last = (CONCTI_MBChain_t*)Z_Malloc(sizeof(*Last), PU_STATIC, NULL);
...
static bool CONLS_InitConsole(CONL_BasicBuffer_t* const a_Buffer, const uint32_t a_Size, const CONL_FlushFunc_t a_FlushFunc)
{
	uint32_t Size, i;
	...
	a_Buffer->Buffer = (char*)Z_Malloc(sizeof(*a_Buffer->Buffer) * a_Buffer->Size, PU_STATIC, NULL);
	
	// Lines
	a_Buffer->NumLines = 128;
	a_Buffer->MaskLine = a_Buffer->NumLines - 1;
	a_Buffer->Lines = (char**)Z_Malloc(sizeof(*a_Buffer->Lines) * a_Buffer->NumLines, PU_STATIC, NULL);
	...
	for (j = 0, n = a_Buf; *n;)
	{
		...
		for (Quote = 0, p = m; p != n; p++)
		{
			...
			if (Current == ArgC)
			{
				...
				// Create text here
				j = 0;
				ArgV[Current] = (char*)Z_Malloc(sizeof(*ArgV[Current]) * MAXARGV, PU_STATIC, NULL);
				Quote = 0;
			}
...
static bool CONL_OutBack(struct CONCTI_Inputter_s* a_Input, const char* const a_Buf)
{
	...
	for (RetVal = 0, p = a_Text; *p; p++, RetVal++)
	{
		...
		if (*p == '\n'/* || (a_Buffer->ExtraNewLine && *p == a_Buffer->ExtraNewLine)*/)
		{
			...
			if (a_Buffer->FlushFunc)
			{
				...
				{
					// Allocate buffer
					Que[Q] = (char*)Z_Malloc(j + 2, PU_STATIC, NULL);
...
} l_OSKLayout[LOSKROWS][LOSKCOLS] =	//
{
	// ` 1 2 3 4 5 6 7 8 9 0 - =  BP IN HM PU 14/3 keys 
	{
		...
		{1, 1, {IKBK_BACKSPACE, IKBK_NULL, IKBK_NULL}, {"\xE2\x8C\xAB", NULL, NULL}},
		
		{1, 1, {IKBK_INSERT, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
		{1, 1, {IKBK_HOME, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
		{1, 1, {IKBK_PAGEUP, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
	},
	
	// T  q w e r t y u i o p [ ] \  DL EN PD 14/3 keys
	{
		{1, 1, {IKBK_TAB, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
		...
		{1, 1, {IKBK_KDELETE, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
		{1, 1, {IKBK_END, IKBK_NULL, IKBK_NULL}, {"#N", NULL, NULL}},
		{1, 1, {IKBK_PAGEDOWN, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
	},
	
	// CL  a s d f g h j k l ; ' RET  13 keys
	{
		{1, 1, {IKBK_CAPSLOCK, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
		...
		{2, 1, {IKBK_RETURN, IKBK_NULL, IKBK_NULL}, {"RT", NULL, NULL}},
		{0},
	},
	
	// SH   z x c v b n m , . /   SH    UA    12/1 keys
	{
		{2, 1, {IKBK_SHIFT, IKBK_NULL, IKBK_NULL}, {"SH", NULL, NULL}},
		{0},
		...
		{0},
		{1, 1, {IKBK_UP, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
		{0},
	},
	
	// CT AL      space        AL CT  5 keys
	{
		{2, 1, {IKBK_CTRL, IKBK_NULL, IKBK_NULL}, {"CT", NULL, NULL}},
		{0},
		{2, 1, {IKBK_ALT, IKBK_NULL, IKBK_NULL}, {"AL", NULL, NULL}},
		{0},
		{6, 1, {IKBK_SPACE, IKBK_NULL, IKBK_NULL}, {" ", " ", " "}},
		{0},
		{0},
		{0},
		{0},
		...
		{1, 1, {IKBK_LEFT, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
		{1, 1, {IKBK_DOWN, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
		{1, 1, {IKBK_RIGHT, IKBK_NULL, IKBK_NULL}, {"#", NULL, NULL}},
	},
};

static int8_t l_OSKSel[MAXSPLITSCREEN][2];
static uint32_t l_OSKLast[MAXSPLITSCREEN][2];
static bool l_OSKVis[MAXSPLITSCREEN];
...
bool CONL_DrawConsole(void)
{
	...
	else if (CONL_IsActive())
	{
		...
		bw = V_FontWidth((VideoFont_t)l_CONFont.Value[0].Int);
		bh = V_FontHeight((VideoFont_t)l_CONFont.Value[0].Int);
		...
		for (l = 0, i = 0, j = ((Out->EndLine - 1) & Out->MaskLine); i < NumLines; j = ((j - 1) & Out->MaskLine))
		{
			...
			while (*p && *p != '\n' && x < conW && y > conY)
			{
				...
				else if (*p > 7)
				{
					...
					bx = V_DrawCharacterMB((VideoFont_t)l_CONFont.Value[0].Int, Options, TempFill, x, y, &BSkip, &Options);
				}
		...
		if (!con_startup)
		{
			...
			// Draw command prompt
			x += V_DrawStringA((VideoFont_t)l_CONFont.Value[0].Int, VFO_COLOR(VEX_MAP_BRIGHTWHITE) | Options, "#>", x, y);
			
			// Draw input buffer
			Options |= VFO_COLOR(VEX_MAP_GREEN);
			l_CONLInputter->Font = (VideoFont_t)l_CONFont.Value[0].Int;
...
static bool l_CLSDoBase = false;
static bool l_CLSBaseDrawn = false;
...
static void CONLS_DrawLoadingScreen(const bool a_QuickDraw)
{
	size_t i;
	uint32_t BarBits;
	fixed_t Frac;
