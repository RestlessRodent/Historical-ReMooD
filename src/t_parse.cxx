...
static void next_token()
{
	...
	{
		tt = isop(*rover) ? fragglescript_operator                : isnum(*rover) ? number : name;
	}
}
...
int find_operator(int start, int stop, char* value)
{
	...
	for (i = start; i <= stop; i++)
	{
		...
		if (tokentype[i] != fragglescript_operator)
			continue;
...
svalue_t evaluate_expression(int start, int stop)
{
	...
	if (tokentype[start] == fragglescript_operator && tokentype[stop] == fragglescript_operator)
