...
static void readthing(MYFILE * f, int num)
{
	...
	do
	{
		if (myfgets(s, sizeof(s), f) != NULL)
		{
			...
			else if (!strcmp(word, "First"))
				mobjinfo[num].seestate = value;
			else if (!strcmp(word, "Alert"))
			...
			else if (!strcmp(word, "Bits"))
				mobjinfo[num].flags = value;
			else if (!strcmp(word, "Bits2"))
				mobjinfo[num].flags2 = value;
			else if (!strcmp(word, "Respawn"))
				mobjinfo[num].raisestate = value;
			else
				deh_error("Thing %d : unknow word '%s'\n", num, word);
		}
	}
		...
		if (myfgets(s, sizeof(s), f) != NULL)
		{
			...
			if (!strcmp(word1, "Sprite"))
			{
				if (!strcmp(word2, "number"))
					states[num].sprite = value;
				else if (!strcmp(word2, "subnumber"))
					states[num].frame = value;
			}
			else if (!strcmp(word1, "Duration"))
				states[num].tics = value;
			else if (!strcmp(word1, "Next"))
				states[num].nextstate = value;
			else
				deh_error("Frame %d : unknow word '%s'\n", num, word1);
		}
	}
	while (s[0] != '\n' && !myfeof(f));
}
	...
	do
	{
		if (myfgets(s, sizeof(s), f) != NULL)
		{
			...
			if (!strcmp(word, "Ammo"))
			{
				...
				wpnlev2info[num].ammo = value;
			}
			else if (!strcmp(word, "Deselect"))
			{
				wpnlev1info[num].upstate = value;
				wpnlev2info[num].upstate = value;
			}
			else if (!strcmp(word, "Select"))
			{
				wpnlev1info[num].downstate = value;
				wpnlev2info[num].downstate = value;
			}
			else if (!strcmp(word, "Bobbing"))
			{
				wpnlev1info[num].readystate = value;
				wpnlev2info[num].readystate = value;
			}
			else if (!strcmp(word, "Shooting"))
			{
				wpnlev1info[num].atkstate = wpnlev1info[num].holdatkstate = value;
				wpnlev2info[num].atkstate = wpnlev2info[num].holdatkstate = value;
			}
			else if (!strcmp(word, "Firing"))
			{
				wpnlev1info[num].flashstate = value;
				wpnlev2info[num].flashstate = value;
			}
			else
				deh_error("Weapon %d : unknow word '%s'\n", num, word);
		}
	}
	while (s[0] != '\n' && !myfeof(f));
}
	...
	do
	{
		if (myfgets(s, sizeof(s), f) != NULL)
		{
			...
			else if (!strcmp(word, "Max"))
			{
				if (!strcmp(word2, "Health"))
					MAXHEALTH = value;
				else if (!strcmp(word2, "Armor"))
					max_armor = value;
				else if (!strcmp(word2, "Soulsphere"))
					maxsoul = value;
			}
			...
			else if (!strcmp(word, "Soulsphere"))
				soul_health = value;
			else if (!strcmp(word, "Megasphere"))
				mega_health = value;
			else if (!strcmp(word, "God"))
				god_health = value;
...
static void readcheat(MYFILE * f)
{
	...
	do
	{
		if (myfgets(s, sizeof(s), f) != NULL)
		{
			...
			else if (!strcmp(word, "Berserk"))
				change_cheat_code(cheat_powerup_seq[1], value);
			else if (!strcmp(word, "Invisibility"))
				change_cheat_code(cheat_powerup_seq[2], value);
			else if (!strcmp(word, "Radiation"))
				change_cheat_code(cheat_powerup_seq[3], value);
			else if (!strcmp(word, "Auto-map"))
				change_cheat_code(cheat_powerup_seq[4], value);
			else if (!strcmp(word, "Lite-Amp"))
				change_cheat_code(cheat_powerup_seq[5], value);
			else if (!strcmp(word, "BEHOLD"))
				change_cheat_code(cheat_powerup_seq[6], value);
			else if (!strcmp(word, "Level"))
				change_cheat_code(cheat_clev_seq, value);
			else if (!strcmp(word, "Player"))
				change_cheat_code(cheat_mypos_seq, value);
			else if (!strcmp(word, "Map"))
				change_cheat_code(cheat_amap_seq, value);
			else
				deh_error("Cheat : unknow word '%s'\n", word);
		}
	}
	while (s[0] != '\n' && !myfeof(f));
}
