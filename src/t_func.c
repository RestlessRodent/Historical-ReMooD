...
void SF_PlayerAmmo()
{
	...
	else
	{
		...
		newammo =
			newammo >
			players[playernum].maxammo[ammonum] ? players[playernum].maxammo[ammonum] : newammo;
		t_return.type = svt_int;
...
void SF_SetCamera()
{
	...
	script_camera.mo->z =
		t_argc <
		3 ? (mo->subsector->sector->floorheight + (41 << FRACBITS)) : fixedvalue(t_argv[2]);
...
void SF_MoveCamera()
{
	...
	if ((quad1 > quad2 && quad1 - 1 == quad2) ||
		(quad2 > quad1 && quad2 - 1 == quad1) || quad1 == quad2)
	{
		...
		while ((i = P_FindSectorFromTag(tagnum, i)) >= 0)
		{
			...
			if (T_MovePlane
				(&sectors[i],
				 ...
				 fixedvalue(t_argv[1]), crush, 0,
				 fixedvalue(t_argv[1]) > sectors[i].floorheight ? 1 : -1) == crushed)
				returnval = 0;
		}
	}
	else
		...
		while ((i = P_FindSectorFromTag(tagnum, i)) >= 0)
		{
			...
			if (T_MovePlane
				(&sectors[i],
				 ...
				 fixedvalue(t_argv[1]) > sectors[i].ceilingheight ? 1 : -1) == crushed)
				returnval = 0;
		}
	}
	else
...
void SF_NewHUPic()
{
	...
	t_return.type = svt_int;
	t_return.value.i =
		HU_GetFSPic(W_GetNumForName(stringvalue(t_argv[0])),
...
void SF_ModifyHUPic()
{
	...
	if (HU_ModifyFSPic
		(intvalue(t_argv[0]), W_GetNumForName(stringvalue(t_argv[1])),
