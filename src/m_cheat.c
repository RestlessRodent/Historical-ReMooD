...
boolean cht_Responder(event_t * ev)
{
	...
	if (ev->type == ev_keydown)
	{
		...
		else if (cht_CheckCheat(&cheat_ammonokey, ev->data1))
		{
			...
			for (i = 0; i < wp_staff; i++)
				plyr->weaponowned[i] = true;

			for (i = 0; i < am_goldwand; i++)
	...
	for (k = 0; k < (cv_splitscreen.value ? 2 : 1); k++)
	{
		...
		for (i = 1; i < COM_Argc(); i++)
		{
			...
			if (all || !strncmp(s, "weapons", 7))
			{
				if (gamemode == heretic)
				{
					for (j = wp_staff; j < NUMWEAPONS; j++)
						plyr->weaponowned[j] = true;

					for (j = am_goldwand; j < NUMAMMO; j++)
						plyr->ammo[j] = plyr->maxammo[j];
				}
				else
				{
					for (j = 0; j < wp_staff; j++)
						if ((gamemission == doom && j != wp_supershotgun) || (gamemission != doom))
							plyr->weaponowned[j] = true;

					for (j = 0; j < am_goldwand; j++)
						plyr->ammo[j] = plyr->maxammo[j];
				}

				CONS_Printf("got weapons\n");
			}
			if (!strncmp(s, "otherweapons", 7))
			{
				if (gamemode != heretic)
				{
					for (j = wp_staff; j < NUMWEAPONS; j++)
			...
			if (all || !strncmp(s, "tomeofpower", 11))
			{
				plyr->powers[pw_weaponlevel2] = WPNLEV2TICS;
				plyr->weaponinfo = wpnlev2info;

				CONS_Printf("got tomeofpower\n");
			}
...
static boolean HandleCheats(byte key)
{
	...
	if (players[consoleplayer[0]].health <= 0)
	...
	for (i = 0; Cheats[i].func != NULL; i++)
	{
		...
		{
			Cheats[i].func(&players[consoleplayer[0]], &Cheats[i]);
...
static void CheatWeaponsFunc(player_t * player, Cheat_t * cheat)
{
	...
	if (!player->backpack)
	{
		for (i = am_goldwand; i < NUMAMMO; i++)
		{
			player->maxammo[i] *= 2;
		}
		player->backpack = true;
	}
	for (i = wp_staff; i < NUMWEAPONS; i++)
	{
