...
#include "m_random.h"
#include "d_clisrv.h"
#include "../src_cl/r_splats.h"			//faB: in dev.
#include "p_demcmp.h"
...
void P_XYFriction(mobj_t * mo, fixed_t oldx, fixed_t oldy, boolean oldfriction)
{
									...
									(player->cmd.forwardmove == 0 && player->cmd.sidemove == 0)))
	{
		...
		{
			if ((player->chickenTics) && ((unsigned)((player->mo->state - states) - S_CHICPLAY_RUN1) < 4))
				P_SetMobjState(player->mo, S_CHICPLAY);
			else if ((unsigned)((player->mo->state - states) - S_PLAY_RUN1) < 4)
		...
		if (!P_TryMove(mo, ptryx, ptryy, true))	//SoM: 4/10/2000
		{
			...
			else if (mo->flags & MF_MISSILE)
			{
				...
				if (blockingline && demoversion >= 129)	//set by last P_TryMove() that failed
				{
					...
					R_AddWallSplat(blockingline,
								   P_PointOnLineSide(mo->x, mo->y,
													 blockingline), "A_DMG3",
...
// Intercept the stupid 'fall through 3dfloors' bug SSNTails 06-13-2002
	if (mo->subsector->sector->ffloors)
	{
		...
		for (rover = mo->subsector->sector->ffloors; rover; rover = rover->next)
		{
			...
			delta1 =
				mo->z - (*rover->bottomheight + ((*rover->topheight - *rover->bottomheight) / 2));
			delta2 =
				thingtop - (*rover->bottomheight +
	...
	if (mo->z + mo->height > mo->ceilingz)
	{
		...
		if (mo->player && (demoversion >= 112) &&
			!(mo->player->cheats & CF_FLYAROUND) && !(mo->flags2 & MF2_FLY) &&
			mo->momz > 8 * FRACUNIT)
...
void P_NightmareRespawn(mobj_t * mobj)
{
	...
	if (mthing->options & MTF_FS_SPAWNED)
		mo = P_SpawnMobj(mobj->x, mobj->y, mobj->z + (gamemode == heretic ? TELEFOGHEIGHT : 0),
						 MT_TFOG);
	else
		mo = P_SpawnMobj(mobj->x, mobj->y,
						 mobj->subsector->sector->floorheight + (gamemode ==
																 heretic ? TELEFOGHEIGHT : 0),
						 MT_TFOG);
	...
	else if (sector->ffloors)
	{
		...
		for (rover = sector->ffloors; rover; rover = rover->next)
		{
			...
			if (!(oldeflags & (MF_TOUCHWATER | MF_UNDERWATER)) &&
				((mobj->eflags & MF_TOUCHWATER) ||
				 (mobj->eflags & MF_UNDERWATER)) && mobj->type != MT_BLOOD)
...
*/
	...
	if (!(oldeflags & (MF_TOUCHWATER | MF_UNDERWATER)) &&
		...
		mobj->type != MT_BLOOD && demoversion < 132)
			...
			if (z == ONFLOORZ)
				mobj->z =
					R_PointInSubsector(x,
									   y)->sector->floorheight +
					((mobj->spawnpoint->options >> 7) << FRACBITS);
			else if (z == ONCEILINGZ)
				mobj->z =
					R_PointInSubsector(x,
									   y)->sector->ceilingheight -
					(((mobj->spawnpoint->options >> 7) << FRACBITS) - mobj->height);
		}
...
void P_SpawnMapThing(mapthing_t * mthing)
{
	...
	if (gamemode == heretic)
	{
		for (i = BEGINHERETIC_MT; i < ENDHERETIC_MT; i++)
			if (mthing->type == mobjinfo[i].doomednum)
				break;
	}
	else
	{
		for (i = 0; i < BEGINHERETIC_MT; i++)
			if (mthing->type == mobjinfo[i].doomednum)
				break;
	}

	if ((gamemode != heretic && i == BEGINHERETIC_MT) || (gamemode == heretic && i == ENDHERETIC_MT))
	{
		for (i = ENDHERETIC_MT; i < NUMMOBJTYPES; i++)
			if (mthing->type == mobjinfo[i].doomednum)
				break;
...
boolean PTR_BloodTraverse(intercept_t * in)
{
	...
	if (in->isaline)
	{
	  ...
	  hitline:
		...
		if (gamemode == heretic)
			R_AddWallSplat(li, P_PointOnLineSide(bloodspawnpointx, bloodspawnpointy, li), "BLODC0",
						   z, frac, SPLATDRAWMODE_TRANS);
		else
			R_AddWallSplat(li,
					   P_PointOnLineSide(bloodspawnpointx, bloodspawnpointy,
										 li), "BLUDC0", z, frac, SPLATDRAWMODE_TRANS);
		return false;
	}
...
//static int  counter =0;
	...
	for (i = 0; i < numsplats; i++)
	{
		...
		anglesplat =
			angle + (((P_Random() - 128) * FINEANGLES / 512 * anglemul) << ANGLETOFINESHIFT);
...
mobj_t *P_SpawnMissile(mobj_t * source, mobj_t * dest, mobjtype_t type)
{
	...
	if (DEMOCVAR(predictingmonsters).value)	//added by AC for predmonsters
	{
		...
		time =
			P_AproxDistance(dest->x + dest->momx * time - source->x,
							dest->y + dest->momy * time - source->y) / th->info->speed;
...
if (XFlagConvList[i].nf && (MObj->mf & XFlagConvList[i].nf))\
	...
	for (i = 0; i < n; i++)
	{
		REMOOD_NFTOXFGROUP(Flag,flags)
		REMOOD_NFTOXFGROUP(Flag2,flags2)
		REMOOD_NFTOXFGROUP(FlagE,eflags)
	}
...
if (XFlagConvList[i].xf && (MObj->mf & XFlagConvList[i].xf))\
	...
	for (i = 0; i < n; i++)
	{
		REMOOD_XFTONFGROUP(XFlagA,XFlagsA)
		REMOOD_XFTONFGROUP(XFlagB,XFlagsB)
		REMOOD_XFTONFGROUP(XFlagC,XFlagsC)
		REMOOD_XFTONFGROUP(XFlagD,XFlagsD)
	}
