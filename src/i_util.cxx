...
/* My stuff */
static bool l_MouseOK = false;				// OK to use the mouse code?
static bool l_NoMouseGrab = false;			// Don't grab mouse
...
static bool l_JoyOK = false;	// Joysticks OK
...
void I_DoMouseGrabbing(void)
{
	...
	New = !(dedicated || gamestate == GS_WAITINGPLAYERS || gamestate == GS_WAITFORJOINWINDOW || gamestate == GS_DEMOSCREEN || demoplayback || M_ExUIActive() || CONL_IsActive());
...
bool VID_AddMode(const int a_Width, const int a_Height, const bool a_Fullscreen)
{
	...
	l_Modes = (I_VideoMode_t*)I_SysRealloc(l_Modes, sizeof(*l_Modes) * (l_NumModes + 1));
...
void I_VideoSetBuffer(const uint32_t a_Width, const uint32_t a_Height, const uint32_t a_Pitch, uint8_t* const a_Direct)
{
	...
	vid.buffer = (uint8_t*)I_SysAlloc(a_Width * a_Height * NUMSCREENS);
...
void I_ShutdownSystem(void)
{
	...
	for (c = 0; c < 2; c++)
	{
		...
		// Get size and data
		Temp = (uint8_t*)WX_CacheEntry(Entry);
		Size = WX_GetEntrySize(Entry);
		
		// Duplicate
		EDData[c] = (uint8_t*)I_SysAlloc(4000);
