...
{
	{IT_WHITESTRING | IT_CALL, NULL,			PTROFUNICODESTRING(MENU_PROFILES_CREATEPROFILE), M_CreateProfile},
	{IT_STRING | IT_CVAR, NULL,					PTROFUNICODESTRING(MENU_PROFILES_CURRENTPROFILE), &DynProfileChooser},
	{IT_STRING | IT_SPACE, NULL,				PTROFUNICODESTRING(MENU_NULLSPACE), NULL},
	{IT_STRING | IT_CVAR | IT_CV_STRING, NULL,	PTROFUNICODESTRING(MENU_PROFILES_NAME), NULL},
	{IT_STRING | IT_CVAR, NULL,					PTROFUNICODESTRING(MENU_PROFILES_COLOR), NULL},
	{IT_STRING | IT_KEYHANDLER, NULL,			PTROFUNICODESTRING(MENU_PROFILES_SKIN), M_HandleSkinChanger},
	{IT_STRING | IT_CVAR, NULL,					PTROFUNICODESTRING(MENU_PROFILES_AUTOAIM), NULL},
};
...
menu_t ProfileDef =
{
	...
	"M_OPTION",
	PTROFUNICODESTRING(MENU_PROFILES_TITLE),
...
{
	{IT_SPACE | IT_STRING | IT_CENTERSTRING, NULL, PTROFUNICODESTRING(MENU_CREATEPROFILE_PLEASENAME), NULL},
	{IT_STRING | IT_CVAR | IT_CV_STRING, NULL, PTROFUNICODESTRING(MENU_CREATEPROFILE_NAME), &TempProfileName},
	{IT_WHITESTRING | IT_CALL, NULL, PTROFUNICODESTRING(MENU_CREATEPROFILE_ACCEPT), M_AcceptNewProfile},
};
...
menu_t CreateProfileDef =
{
	0,
	NULL,
	PTROFUNICODESTRING(MENU_CREATEPROFILE_TITLE),
...
void M_DrawProfileMenu(void)
{
	...
	V_DrawString(
		(currentMenu->x + currentMenu->width) - V_StringWidth(skinchar),
		(currentMenu->y + (5 * STRINGHEIGHT)), V_WHITEMAP, skinchar);
}
...
menuitem_t ProfileChooserItems[] =
{
	{IT_SPACE | IT_STRING | IT_CENTERSTRING, NULL,
		PTROFUNICODESTRING(MENU_SELECTPROFILE_PLEASESELECT), NULL},
	{IT_SPACE | IT_STRING | IT_CENTERSTRING, NULL,
		PTROFUNICODESTRING(MENU_SELECTPROFILE_PLACEHOLDER), NULL},
	{IT_STRING | IT_CVAR, NULL, PTROFUNICODESTRING(MENU_SELECTPROFILE_PROFILE), &InGameProfileChooser},
	{IT_WHITESTRING | IT_CALL, NULL, PTROFUNICODESTRING(MENU_SELECTPROFILE_ACCEPT), M_ProfAccept},
};
...
menu_t ProfileChooser =
{
	0,
	NULL,
	PTROFUNICODESTRING(MENU_SELECTPROFILE_TITLE),
...
void M_ProfilePrompt(int player)
{
	...
	{
		if (i == 0)
			ProfileChooserItems[1].WItemTextPtr = PTROFUNICODESTRING(MENU_SELECTPROFILE_TWOSPLITA);
		else
			ProfileChooserItems[1].WItemTextPtr = PTROFUNICODESTRING(MENU_SELECTPROFILE_TWOSPLITB);
	}
	else if (cv_splitscreen.value > 1)
	{
		if (i == 0)
			ProfileChooserItems[1].WItemTextPtr = PTROFUNICODESTRING(MENU_SELECTPROFILE_FOURSPLITA);
		else if (i == 1)
			ProfileChooserItems[1].WItemTextPtr = PTROFUNICODESTRING(MENU_SELECTPROFILE_FOURSPLITB);
		else if (i == 2)
			ProfileChooserItems[1].WItemTextPtr = PTROFUNICODESTRING(MENU_SELECTPROFILE_FOURSPLITC);
		else
			ProfileChooserItems[1].WItemTextPtr = PTROFUNICODESTRING(MENU_SELECTPROFILE_FOURSPLITD);
	}
	else
		ProfileChooserItems[1].WItemTextPtr = PTROFUNICODESTRING(MENU_SELECTPROFILE_FORYOU);
