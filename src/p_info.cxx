...
typedef struct P_LevelInfoHolder_s
{
	...
	bool IWADParsed;							// IWAD info parsed in
...
static bool PS_ParseMapInfo(P_LevelInfoHolder_t* const a_Holder, const WL_WADEntry_t* a_MIEntry)
{
#define BUFSIZE 512
	char Buf[BUFSIZE];
	char Token[BUFSIZE];
...
static bool P_WLInfoCreator(const WL_WADFile_t* const a_WAD, const uint32_t a_Key, void** const a_DataPtr, size_t* const a_SizePtr, WL_RemoveFunc_t* const a_RemoveFuncPtr)
{
	size_t i, j;
	...
	*a_DataPtr = Holder = (P_LevelInfoHolder_t*)Z_Malloc(*a_SizePtr, PU_WLDKRMOD, NULL);
	...
	for (i = 0; i < a_WAD->NumEntries; i++)
	{
		...
		if (!LoadLumps)
		{
			...
			CurrentInfo = Holder->Infos[Holder->NumInfos++] = (P_LevelInfoEx_t*)Z_Malloc(sizeof(*CurrentInfo), PU_WLDKRMOD, NULL);
...
static bool PS_WLInfoOCCB(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
	...
	// Get holder
	Holder = (P_LevelInfoHolder_t*)WL_GetPrivateData(Rover, WLDK_MAPINFO, &Sz);
	...
	if (Holder && Rover->__Private.__IsIWAD && !Holder->IWADParsed)
	{
		...
		Entry = WL_FindEntry((const WL_WADFile_t*)g_ReMooDPtr, 0, g_IWADMapInfoName);
	...
	for (Rover = WL_IterateVWAD(NULL, true); Rover; Rover = WL_IterateVWAD(Rover, true))
	{
		...
		for (i = 0; i < Holder->NumInfos; i++)
		{
			...
			if (CompInfo)
			{
				...
				{
					// Allocate
					Composite = (P_LevelInfoEx_t*)Z_Malloc(sizeof(*Composite), PU_WLDKRMOD, NULL);
