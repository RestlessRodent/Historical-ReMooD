...
void R_LoadTextures(void)
{
	...
	for (i = 0; i < numtextures; i++, directory++)
	{
		...
		textures[i] =
			Z_Malloc(sizeof(texture_t) + sizeof(texpatch_t) * (mtexture->patchcount), PU_STATIC, 0);
		/*if (devparm)
		...
		for (j = 0; j < texture->patchcount; j++, mpatch++, patch++)
		{
			...
			{
				I_Error
					("R_InitTextures: Missing patch in texture %s (tried %i)",
					 texture->name, mpatch->patch);
			}
		}
...
void R_InitExtraColormaps()
{
	...
	for (i = 0; i < W_NumWadFiles(); i++, clump = 0)
	{
		...
		colormaplumps =
			(lumplist_t *) realloc(colormaplumps, sizeof(lumplist_t) * (numcolormaplumps + 1));
...
int R_ColormapNumForName(char *name)
{
	...
	extra_colormaps[num_extra_colormaps].colormap =
		Z_MallocAlign(W_LumpLength(lump), PU_LEVEL, 0, 8);
...
#undef HEX2INT
	...
	for (i = 0; i < num_extra_colormaps; i++)
	{
		...
		if (maskcolor == extra_colormaps[i].maskcolor &&
			...
			fadeend == extra_colormaps[i].fadeend && fog == extra_colormaps[i].fog)
			return i;
	}
...
#define ABS2(x) ((x) < 0 ? -(x) : (x))
	...
	for (p = 0; p < 34; p++)
	{
		...
		{
			*colormap_p =
				NearestColor(RoundUp(map[i][0]), RoundUp(map[i][1]), RoundUp(map[i][2]));
			colormap_p++;
...
//  char*               flatpresent; //SoM: 4/18/2000: No longer used
	...
	if (devparm)
	{
		CONS_Printf("Precache level done:\n"
					...
					"texturememory: %ld k\n"
					"spritememory:  %ld k\n", flatmemory >> 10,
					texturememory >> 10, spritememory >> 10);
	}
}
