...
void D_Display(void)
{
	...
	switch (gamestate)
	{
		...
		case GS_DEMOSCREEN:
			D_PageDrawer(g_Globals->pagename);
		case GS_NULL:
			break;
	}
...
void D_PageDrawer(char *lumpname)
{
	...
	if (raven && g_Globals->demosequence != 2)	// big hack for legacy's credits
	{
		V_DrawRawScreen(0, 0, W_GetNumForName(lumpname), 320, 200);
		if (g_Globals->demosequence == 0 && g_Globals->pagetic <= 140)
...
void D_DoomMain(void)
{
	...
	// GhostlyDeath <March 12, 2011> -- Initialize globals
	ReMooD_GlobalsNew();
