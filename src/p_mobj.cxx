...
bool P_SetMobjState(mobj_t* mobj, statenum_t state)
{
	...
	tempstate = (statenum_t*)Z_Malloc(sizeof(*tempstate) * NUMSTATES, PU_STATIC, NULL);
	...
	if (!seenstate_tab || OldNumStates != NUMSTATES)
	{
		...
		seenstate_tab = (statenum_t*)Z_Malloc(sizeof(*seenstate_tab) * NUMSTATES, PU_STATIC, NULL);
			...
			else if (mo->flags & MF_MISSILE)
			{
				...
				if (blockingline && P_EXGSGetValue(PEXGSBID_COENABLEBLOODSPLATS))	//set by last P_TryMove() that failed
				{
					...
					R_AddWallSplat(
							...
							(mo->info->RMissileSplat ? mo->info->RMissileSplat : (char*)"A_DMG3"),
...
static bool PIT_StackThings(mobj_t* thing, void* a_Arg)
{
	fixed_t blockdist;
	mobj_t* SourceMo;
	size_t i, j;
...
mobj_t* P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
	...
	if (mobj->RXFlags[0] & MFREXA_USENULLMOTHINKER)
		mobj->thinker.function.acv = (void (*)())((actionf_p1) P_MobjNullThinker);
	else
	{
...
void P_RemoveMobj(mobj_t* mobj)
{
	...
	for (i = 0; i < NUMPMOBJREFTYPES; i++)
		P_RefMobj((P_MobjRefType_t)i, mobj, NULL);
	...
	for (i = 0; i < NUMPMOBJREFTYPES; i++)
		if (mobj->RefCount[i] > 0)
		{
			...
			P_FindMobjRef((P_MobjRefType_t)i, mobj);
...
bool PTR_BloodTraverse(intercept_t* in, void* const a_Data)
{
	...
	divline_t divl;
	fixed_t frac;
	mobj_t* BleedThing = (mobj_t*)a_Data;
...
hitline:
		...
		R_AddWallSplat(
				...
				(BleedThing && BleedThing->info->RBloodSplat ? BleedThing->info->RBloodSplat : (char*)"BLUDC0"),
				z,
				frac,
				SPLATDRAWMODE_TRANS
			);
...
void P_ClearMobjRefs(mobj_t* const a_Mo)
{
	...
	for (i = 0; i < NUMPMOBJREFTYPES; i++)
		for (j = 0; j < a_Mo->RefListSz[i]; j++)
			if (a_Mo->RefList[i][j])
				P_RefMobj((P_MobjRefType_t)i, a_Mo->RefList[i][j], NULL);
}
...
void P_MorphObjectClass(mobj_t* const a_Mo, const mobjtype_t a_NewClass)
{
	...
	OldGroup = (INFO_ObjectStateGroup_t)((a_Mo->state->Marker & 0xFFFF0000U) >> 16U);
...
{
	bool IsThisPlayer, IsOtherPlayer;
