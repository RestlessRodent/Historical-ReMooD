...
static bool RS_GetMarkers(const WL_WADFile_t* const a_WAD, const char* const a_Start, const char* const a_End, const WL_WADEntry_t** const a_SOut, const WL_WADEntry_t** const a_EOut)
{
...
static bool RS_TexturePDCreate(const struct WL_WADFile_s* const a_WAD, const uint32_t a_Key, void** const a_DataPtr, size_t* const a_SizePtr, WL_RemoveFunc_t* const a_RemoveFuncPtr)
{
#define TEMPNAME 9
	...
	*a_DataPtr = Holder = (R_TextureHolder_t*)Z_Malloc(*a_SizePtr, PU_WLDKRMOD, NULL);
	...
	if (Entry)
	{
		...
		if (Stream)
		{
			...
			Holder->Patches = (char**)Z_Malloc(sizeof(*Holder->Patches) * Holder->NumPatches, PU_WLDKRMOD, NULL);
	...
	for (BaseOffset = 0, i = 0; i <= 9; i++)
	{
		...
		for (z = BaseOffset, j = 0; j < ThisCount; j++, z++)
		{
			...
			if (Holder->Textures[z].patchcount)
				Holder->Textures[z].patches = (texpatch_t*)Z_Malloc(sizeof(*Holder->Textures[z].patches) * Holder->Textures[z].patchcount, PU_WLDKRMOD, NULL);
	...
	if (MStart && MEnd)
	{
		...
		for (i = BaseOffset, Entry = MStart; Entry && Entry != MEnd; i++, Entry = Entry->NextEntry)
		{
			...
			// Set entry to this
			Holder->Textures[i].FlatEntry = (void*)Entry;
		}
	}
	...
	if (MStart && MEnd)
	{
		...
		Holder->SpriteInfo = (R_SpriteInfoEx_t*)Z_Malloc(sizeof(*Holder->SpriteInfo) * Holder->NumSpriteInfo, PU_WLDKRMOD, (void**)&Holder->SpriteInfo);
		...
		for (i = 0, Entry = MStart; Entry && Entry != MEnd; Entry = Entry->NextEntry)
		{
			...
			// Place entry
			Holder->SpriteInfo[i].Entry = (WL_WADEntry_s*)Entry;
...
{
	bool AnyMode;								// Find anything
	bool CheckFlat;							// Check for flat
...
// A = R_TCFInput_t*
// B = uintptr_t
static bool RS_TextureCompareFunc(void* const a_A, void* const a_B)
{
	R_TCFInput_t* TCF;
	uintptr_t TextID;
...
static bool RS_HashUpTextures(void)
{
	size_t i;
	uint32_t Hash;
	...
	for (i = 0; i < numtextures; i++)
	{
		...
		Z_HashAddEntry(l_TextureHashes, Hash, (void*)((uintptr_t)(i + 1)));
	}
...
static bool RS_TextureOrderChange(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
	R_TCFInput_t TCF;
	...
	{
		// Obtain texture info
		Holder = (R_TextureHolder_t*)WL_GetPrivateData(Rover, WLDK_TEXTURES, &HolderSize);
	...
	TempArray = (texture_t**)Z_Malloc(sizeof(*TempArray) * numtextures, PU_WLDKRMOD, NULL);
	...
	for (Rover = WL_IterateVWAD(NULL, true); Rover; Rover = WL_IterateVWAD(Rover, true))
	{
		...
		for (i = 0; i < Holder->NumSpriteInfo; i++)
		{
			...
			if (!ThisDef)
			{
				...
				ThisDef->spriteframes = (spriteframe_t*)Z_Malloc(sizeof(*ThisDef->spriteframes) * ThisDef->numframes, PU_WLDKRMOD, NULL);
			}
	...
	sprites = (spritedef_t*)Z_Malloc(sizeof(*sprites) * numsprites, PU_WLDKRMOD, (void**)&sprites);
	g_SprTouchSpecials =
		(void**)Z_Malloc(sizeof(*g_SprTouchSpecials) * numsprites, PU_WLDKRMOD, (void**)&g_SprTouchSpecials);
...
uint8_t* R_GetFlat(int flatlumpnum)
{
	...
	/* Create cache */
	Texture->FlatCache = (uint8_t*)Z_Malloc(w * h, PU_WLDKRMOD, (void**)&Texture->FlatCache);
	...
	if (Texture->IsFlat)
	{
		...
		if (Texture->FlatImage)
			Image = (V_Image_t*)Texture->FlatImage;
		
		// Load image
		else
		{
			Image = V_ImageLoadE((const WL_WADEntry_t*)Texture->FlatEntry, VCP_NONE);
			Texture->FlatImage = Image;
		}
...
uint8_t* R_GenerateTexture(int texnum)
{
	...
	MaskBuffer = NULL;
	Buffer = (uint8_t*)Z_Malloc(sizeof(*Buffer) * (w * h), PU_WLDKRMOD, NULL);
	...
	else
	{
		...
		MaskBuffer = (uint8_t*)Z_Malloc(sizeof(*Buffer) * (w * h), PU_WLDKRMOD, NULL);
	...
	Texture->ColumnOffs = (uint32_t*)Z_Malloc(sizeof(*Texture->ColumnOffs) * w, PU_WLDKRMOD, NULL);
	Texture->CacheSize = (w * (h * 3)) + (6 * w);
	Texture->Cache = (uint8_t*)Z_Malloc(Texture->CacheSize, PU_CACHE, (void**)&Texture->Cache);
...
void R_SetSpriteLumpCount(const size_t a_Count)
{
	...
	/* Resize all arrays */
	Z_ResizeArray((void**)&spritewidth, sizeof(*spritewidth) * NUMSPRITEBUMPS, g_SpritesBufferSize, BufferMul);
	Z_ResizeArray((void**)&spriteoffset, sizeof(*spriteoffset) * NUMSPRITEBUMPS, g_SpritesBufferSize, BufferMul);
	Z_ResizeArray((void**)&spritetopoffset, sizeof(*spritetopoffset) * NUMSPRITEBUMPS, g_SpritesBufferSize, BufferMul);
	Z_ResizeArray((void**)&spriteheight, sizeof(*spriteheight) * NUMSPRITEBUMPS, g_SpritesBufferSize, BufferMul);
...
static bool RS_ColormapOCCB(const bool a_Pushed, const struct WL_WADFile_s* const a_WAD)
{
	const WL_WADEntry_t* Entry;
	...
	if (Entry)
	{
		...
		// Allocate new space
		colormaps = (lighttable_t*)Z_Malloc(Entry->Size, PU_WLDKRMOD, NULL);
