...
void T_PlatRaise(plat_t* plat)
{
	...
	switch (plat->status)
	{
		case up:
			...
			{
				if (!(leveltime % (8)))
					S_StartSound((S_NoiseThinker_t*)&plat->sector->soundorg, sfx_stnmov);
			}
			...
			if (res == crushed && (!plat->crush))
			{
				...
				plat->status = down;
				S_StartSound((S_NoiseThinker_t*)&plat->sector->soundorg, sfx_pstart);
			}
			else
			{
				if (res == pastdest)
				{
					...
					if (plat->type != toggleUpDn)
					{
						plat->count = plat->wait;
						plat->status = waiting;
						S_StartSound((S_NoiseThinker_t*)&plat->sector->soundorg, sfx_pstop);
					}
	...
	while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
	{
		...
		switch (type)
		{
			case raiseToNearestAndChange:
				...
				S_StartSound((S_NoiseThinker_t*)&sec->soundorg, sfx_stnmov);
				break;
