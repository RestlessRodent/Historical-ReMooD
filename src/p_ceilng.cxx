		...
		case 1:
			...
			if (!(leveltime % (8)))
			{
				switch (ceiling->type)
				{
					...
					case genSilentCrusher:
						break;
					default:
						S_StartSound((S_NoiseThinker_t*)&ceiling->sector->soundorg, ceilmovesound);
						// ?
						break;
				}
			}
			...
			if (res == pastdest)
			{
				switch (ceiling->type)
				{
					...
					case silentCrushAndRaise:
						//S_StartSound((S_NoiseThinker_t*)&ceiling->sector->soundorg, sfx_pstop);
...
{
	ceilinglist_t* list = (ceilinglist_t*)Z_Malloc(sizeof *list, PU_STATIC, NULL);
