...
struct WL_EntryStream_s
{
	...
	bool IsUnicode;							// Is UTF16 stream
	bool IsSwapped;							// Is byte swapped
...
WL_EntryStream_t* WL_StreamOpen(const WL_WADEntry_t* const a_Entry)
{
	...
	/* Create a new stream */
	New = (WL_EntryStream_t*)Z_Malloc(sizeof(*New), PU_STATIC, NULL);
	
	/* Set info */
	New->Entry = a_Entry;
	New->StreamSize = a_Entry->Size;
	New->CacheSize = WLSTREAMCACHESIZE;
	New->Cache = (uint8_t*)Z_Malloc(New->CacheSize, PU_STATIC, NULL);
...
static bool WLS_StreamBufferChunk(WL_EntryStream_t* const a_Stream, const size_t a_Offset)
{
