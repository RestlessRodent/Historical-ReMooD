...
static void R_DrawWallSplats()
{
	...
	for (; splat; splat = splat->next)
	{
		...
		patch = (patch_t*)V_ImageGetPatch(splat->Image, NULL);
...
void R_RenderMaskedSegRange(drawseg_t* ds, int x1, int x2)
{
	...
	if (frontsector->numlights)
	{
		...
		if (dc_numlights >= dc_maxlights)
		{
			...
			dc_lightlist = (r_lightlist_s*)realloc(dc_lightlist, sizeof(r_lightlist_t) * dc_maxlights);
		}
...
void R_StoreWallRange(int start, int stop)
{
	...
	while (ds_p == drawsegs + maxdrawsegs)
	{
		...
		if (!drawsegs)
		{
			drawsegs = (drawseg_t*)Z_Malloc(sizeof(drawseg_t) * 128, PU_STATIC, NULL);
			ds_p = drawsegs + pos;
			...
			for (i = 0; i < maxdrawsegs; i++)
				if (!drawsegs[i].frontscale)
					drawsegs[i].frontscale = (fixed_t*)Z_Malloc(sizeof(*drawsegs[i].frontscale) * (MAXSEGS + 1), PU_STATIC, NULL);
		}
		else
		{
			newseg = (drawseg_t*)Z_Malloc(sizeof(drawseg_t) * newmax, PU_STATIC, NULL);
		...
		if (firstseg)
			firstseg = drawsegs + (intptr_t)firstseg;
	}
