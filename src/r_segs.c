	...
	for (; splat; splat = splat->next)
	{
		...
		switch (splat->flags & SPLATDRAWMODE_MASK)
		{
			...
			case SPLATDRAWMODE_SHADE:
				if (g_PaintBallMode)
				{
					uint8_t DoomColor[11] =
					{112, 160, 176, 200, 215, 231, 251, 83, 168, 172, 194};
...
//                         dc_x,FIXED_TO_FLOAT(rw_offset2),FIXED_TO_FLOAT(splat->offset),angle,FIXED_TO_FLOAT(finetangent[angle]),FIXED_TO_FLOAT(rw_distance),FIXED_TO_FLOAT(FixedMul(finetangent[angle],rw_distance)));
			...
			// draw the texture
			col = (column_t *) ((byte *) patch + LittleSwapInt32(patch->columnofs[texturecolumn]));
			...
			if (dc_numlights)
			{
				...
				for (i = 0; i < dc_numlights; i++)
				{
					...
					if (rlight->extra_colormap && !fixedcolormap)
						rlight->rcolormap =
							rlight->extra_colormap->colormap + (xwalllights[index] - colormaps);
...
void R_RenderThickSideRange(drawseg_t * ds, int x1, int x2, ffloor_t * ffloor)
{
	...
	frontsector =
		curline->frontsector == ffloor->target ? curline->backsector : curline->frontsector;
	...
	if (frontsector->numlights)
	{
		...
		for (i = p = 0; i < dc_numlights; i++)
		{
			...
			rlight->height =
				(centeryfrac) - FixedMul((lheight - viewz), spryscale) - rlight->heightstep;
			rlight->flags = light->flags;
			if (light->flags & FF_CUTLEVEL)
			{
				...
				rlight->botheight =
					(centeryfrac) - FixedMul((lheight - viewz), spryscale) - rlight->botheightstep;
			}
	...
	else
	{
		...
		if ((frontsector->extra_colormap && frontsector->extra_colormap->fog))
			...
			lightnum =
				(R_FakeFlat
				 (frontsector, &tempsec, &templight, &templight,
				  false)->lightlevel >> LIGHTSEGSHIFT) + extralight;
		...
		if (maskedtexturecol[dc_x] != MAXSHORT)
		{
			...
			if (dc_numlights)
			{
				...
				sprtopscreen = windowtop =
					(centeryfrac - FixedMul((dc_texturemid - offsetvalue), spryscale));
				...
				for (i = 0; i < dc_numlights; i++)
				{
					...
					if (lighteffect)
					{
						...
						if (ffloor->flags & FF_FOG)
						{
							if (ffloor->master->frontsector->extra_colormap && !fixedcolormap)
								rlight->rcolormap =
									...
									extra_colormap->colormap + (xwalllights[index] - colormaps);
							else if (!fixedcolormap)
								rlight->rcolormap = xwalllights[index];
						}
						else
						{
							if (rlight->extra_colormap && !fixedcolormap)
								rlight->rcolormap =
									rlight->extra_colormap->colormap +
									(xwalllights[index] - colormaps);
							else if (!fixedcolormap)
								rlight->rcolormap = xwalllights[index];
						}
					...
					else if (rlight->flags & FF_CUTEXTRA && ffloor->flags & FF_EXTRA)
					{
						if (rlight->flags & FF_EXTRA)
						{
							...
							if ((rlight->flags & (FF_TRANSLUCENT | FF_FOG)) ==
								(ffloor->flags & (FF_TRANSLUCENT | FF_FOG)))
								solid = 1;
						}
						else
							solid = 1;
			...
			if (!fixedcolormap)
			{
				...
				if (ffloor->flags & FF_FOG && ffloor->master->frontsector->extra_colormap)
					dc_colormap =
						ffloor->master->frontsector->extra_colormap->colormap +
		...
		if (dc_numlights)
		{
			...
			for (i = 0; i < dc_numlights; i++)
			{
				...
				if ((frontsector->lightlist[i].caster &&
					 ...
					 *frontsector->lightlist[i].caster->bottomheight) ||
				...
				if (dc_lightlist[i].extra_colormap && !fixedcolormap)
					dc_lightlist[i].rcolormap =
						dc_lightlist[i].extra_colormap->colormap + (xwalllights[index] - colormaps);
				else if (!fixedcolormap)
	...
	else
	{
			...
			(backsector->ffloors || frontsector->ffloors))
		{
			...
			lowcut =
				...
				backsector->floorheight ? frontsector->floorheight : backsector->floorheight;
			highcut =
				frontsector->ceilingheight <
				backsector->ceilingheight ? frontsector->ceilingheight : backsector->ceilingheight;
			...
			if (frontsector->ffloors && backsector->ffloors)
			{
				...
				for (rover = backsector->ffloors; rover && i < MAXFFLOORS; rover = rover->next)
				{
					...
					{
						if (!(r2->flags & FF_EXISTS) ||
							!(r2->flags & FF_RENDERSIDES) ||
						...
						if (rover->flags & FF_EXTRA)
						{
							...
							if (r2->flags & FF_EXTRA &&
								(r2->flags & (FF_TRANSLUCENT | FF_FOG)) !=
								(rover->flags & (FF_TRANSLUCENT | FF_FOG)))
								continue;
						}
						else
						{
						...
						if (*rover->topheight > *r2->topheight ||
							*rover->bottomheight < *r2->bottomheight)
							continue;
			...
			{
				for (rover = backsector->ffloors, i = 0;
					 rover && i < MAXFFLOORS; rover = rover->next)
				{
					if (!(rover->flags & FF_RENDERSIDES) ||
						!(rover->flags & FF_EXISTS) || rover->flags & FF_INVERTSIDES)
						continue;
					if (*rover->topheight <= frontsector->floorheight ||
						*rover->bottomheight >= frontsector->ceilingheight)
						continue;
			...
			{
				for (rover = frontsector->ffloors, i = 0;
					 rover && i < MAXFFLOORS; rover = rover->next)
				{
					if (!(rover->flags & FF_RENDERSIDES) ||
						!(rover->flags & FF_EXISTS) || !(rover->flags & FF_ALLSIDES))
						continue;
					if (*rover->topheight <= frontsector->floorheight ||
						*rover->bottomheight >= frontsector->ceilingheight)
						continue;
					if (*rover->topheight <= backsector->floorheight ||
						*rover->bottomheight >= backsector->ceilingheight)
						continue;
		...
		for (i = p = 0; i < dc_numlights; i++)
		{
			...
			if (i != 0)
			{
				...
				if (light->height > frontsector->ceilingheight)
					if (i + 1 < dc_numlights &&
						frontsector->lightlist[i + 1].height > frontsector->ceilingheight)
						continue;
			}
			...
			if (light->caster && light->caster->flags & FF_SOLID)
			{
				lheight =
					*light->caster->bottomheight >
					...
					FRACUNIT : *light->caster->bottomheight;
				rlight->botheight =
					(centeryfrac >> 4) -
					FixedMul((*light->caster->bottomheight - viewz) >> 4, rw_scale);
				rlight->botheightstep =
					-FixedMul(rw_scalestep, (*light->caster->bottomheight - viewz) >> 4);
			}
