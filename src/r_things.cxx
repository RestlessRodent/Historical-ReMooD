...
static void R_DrawVisSprite(vissprite_t* vis, int x1, int x2)
{
	...
	ByteSize = 0;
	patch = (patch_t*)V_ImageGetPatch((V_Image_t*)vis->Image, &ByteSize);
	...
	else
	{
			...
			// because now the translucency is set through FF_TRANSMASK
		{
			if (thing->flags2 & MF2_FORCETRANSPARENCY)
				...
				vis->transmap = (uint8_t*)VIS_OLDFUZZ;
		}
...
void R_DrawPSprite(pspdef_t* psp)
{
	...
	if (viewheight == vid.height || (!ST_ExSoloViewTransSBar && vid.dupy > 1))
...
void R_DrawSprite(vissprite_t* spr)
{
	...
	if (!clipbot)
		clipbot = (short int*)Z_Malloc(sizeof(short) * vid.width, PU_STATIC, (void**)&clipbot);
	if (!cliptop)
		cliptop = (short int*)Z_Malloc(sizeof(short) * vid.width, PU_STATIC, (void**)&clipbot);
