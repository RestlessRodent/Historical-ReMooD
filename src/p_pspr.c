	...
	if (gamemode == heretic)
		do
		{
			...
			else if (player->weaponowned[wp_gauntlets])
			{
				player->pendingweapon = wp_gauntlets;
					 ...
					 player->weaponinfo[wp_phoenixrod].ammopershoot)
			{
		...
		do
		{
			if (player->weaponowned[wp_plasma]
				&& player->ammo[am_cell] >=
				player->weaponinfo[wp_plasma].ammopershoot && (gamemode != shareware))
			{
				player->pendingweapon = wp_plasma;
			}
			else if (player->weaponowned[wp_supershotgun]
					 && player->ammo[am_shell] >=
					 player->weaponinfo[wp_supershotgun].ammopershoot && (gamemode == commercial))
			{
					 ...
					 && player->ammo[am_misl] >= player->weaponinfo[wp_missile].ammopershoot)
			{
				player->pendingweapon = wp_missile;
					 ...
					 && player->ammo[am_cell] >=
					 player->weaponinfo[wp_bfg].ammopershoot && (gamemode != shareware))
			{
...
void P_FireWeapon(player_t * player)
{
	...
	if (player->readyweapon >= wp_staff && player->readyweapon <= wp_beak)
	{
...
void A_WeaponReady(player_t * player, pspdef_t * psp)
{
	...
	if (player->cmd.buttons & BT_ATTACK)
	{
		if (!player->attackdown
			|| (player->readyweapon != wp_missile && (player->readyweapon != wp_bfg || gamemode == heretic)))
		{
...
void A_FirePlasma(player_t * player, pspdef_t * psp)
{
	...
	P_SetPsprite(player,
				 ps_flash, player->weaponinfo[player->readyweapon].flashstate + (P_Random() & 1));
...
void A_FireBlasterPL2(player_t * player, pspdef_t * psp)
{
	...
	if (!cv_infiniteammo.value)
		player->ammo[am_blaster] -= cv_deathmatch.value ? USE_BLSR_AMMO_1 : USE_BLSR_AMMO_2;
...
void A_FireGoldWandPL2(player_t * player, pspdef_t * psp)
{
	...
	if (!cv_infiniteammo.value)
		player->ammo[am_goldwand] -= cv_deathmatch.value ? USE_GWND_AMMO_1 : USE_GWND_AMMO_2;
...
void A_FireMacePL1(player_t * player, pspdef_t * psp)
{
	...
	ball = P_SPMAngle(player->mo, MT_MACEFX1, player->mo->angle + (((P_Random() & 7) - 4) << 24));
	if (ball)
	{
...
void A_FireMacePL2(player_t * player, pspdef_t * psp)
{
	...
	if (!cv_infiniteammo.value)
		player->ammo[am_mace] -= cv_deathmatch.value ? USE_MACE_AMMO_1 : USE_MACE_AMMO_2;
...
void A_FireCrossbowPL2(player_t * player, pspdef_t * psp)
{
	...
	if (!cv_infiniteammo.value)
		player->ammo[am_crossbow] -= cv_deathmatch.value ? USE_CBOW_AMMO_1 : USE_CBOW_AMMO_2;
...
void A_FireSkullRodPL2(player_t * player, pspdef_t * psp)
{
	...
	if (!cv_infiniteammo.value)
		player->ammo[am_skullrod] -= cv_deathmatch.value ? USE_SKRD_AMMO_1 : USE_SKRD_AMMO_2;
