...
typedef struct I_SDLSoundLocal_s
{
	int RetVal;
	SDL_AudioSpec Spec;
	void (*ThreadFunc) (const bool a_Threaded);
	void* Buffer;
	SDL_mutex* Mutex;
} I_SDLSoundLocal_t;
...
bool I_SDLSD_Init(struct I_SoundDriver_s* const a_Driver)
{
	/* Check */
	if (!a_Driver)
		return false;
...
bool I_SDLSD_Destroy(struct I_SoundDriver_s* const a_Driver)
{
	I_SDLSoundLocal_t* Local;
...
size_t I_SDLSD_Request(struct I_SoundDriver_s* const a_Driver, const uint8_t a_Bits, const uint16_t a_Freq, const uint8_t a_Channels, const uint32_t a_Samples)
{
	...
	Local->Spec.callback = (void (*)(void*, Uint8*, int))I_SDLSD_Callback;
...
bool I_SDLSD_IsFinished(struct I_SoundDriver_s* const a_Driver)
{
	I_SDLSoundLocal_t* Local;
	bool RetVal;
...
bool I_SDLSD_Thread(struct I_SoundDriver_s* const a_Driver, void (*a_ThreadFunc) (const bool a_Threaded))
{
	I_SDLSoundLocal_t* Local;
...
void I_SDLSD_LockThread(struct I_SoundDriver_s* const a_Driver, const bool a_Lock)
{
	I_SDLSoundLocal_t* Local;
