...
bool l_AllegroSDMDInitted = false;	// Was install_sound called?

/* I_AllegroSD_Init() -- Initializes a driver */
bool I_AllegroSD_Init(struct I_SoundDriver_s* const a_Driver)
{
	/* Check */
	if (!a_Driver)
		return false;
...
bool I_AllegroSD_Destroy(struct I_SoundDriver_s* const a_Driver)
{
	/* Check */
	if (!a_Driver)
		return false;
...
bool I_AllegroSD_IsFinished(struct I_SoundDriver_s* const a_Driver)
{
	I_AllegroSoundLocal_t* Local;
...
void I_AllegroSD_UnRequest(struct I_SoundDriver_s* const a_Driver)
{
	...
	if (Local->Stream)
	{
		voice_stop((intptr_t)Local->Stream);
		deallocate_voice((intptr_t)Local->Stream);
	}
	// Clear out
	Local->Stream = NULL;
	Local->Buffer = NULL;
