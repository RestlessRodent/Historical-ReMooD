...
extern bool l_AllegroSDMDInitted;	// Was install_sound called?

/* I_AllegroMD_Init() -- Initialize music */
static bool I_AllegroMD_Init(struct I_MusicDriver_s* const a_Driver)
{
	MIDI* mid;
...
static void I_AllegroMD_Stop(struct I_MusicDriver_s* const a_Driver, const int a_Handle)
{
	...
	/* Get Local */
	Local = (I_AllegroMIDILocal_t*)a_Driver->Data;
...
static int I_AllegroMD_Play(struct I_MusicDriver_s* const a_Driver, const void* const a_Data, const size_t a_Size, const bool Loop)
{
	I_AllegroMIDILocal_t* Local;
	...
	Local->CurrentMIDI = load_midi((const char*)a_Data);
...
void I_AllegroMD_RawMIDI(struct I_MusicDriver_s* const a_Driver, const uint32_t a_Msg, const uint32_t a_BitLength)
{
	...
	midi_out((unsigned char*)&a_Msg, (a_BitLength < sizeof(a_Msg) ? a_BitLength : sizeof(a_Msg)));
}
